...
theorem chainPosInv2_alongRun {w : List (Fin 2)} :
  ...
  | @succ n x z y ht _ ih =>
    intro hx hLandingObligations hCanRightAlongRun
    refine ih (chainPosInv2_tick_of_landingObligationsAt centre place entry q first
        (hLandingObligations 0 x (.zero x))
        (hCanRightAlongRun 1 z (.succ ht (.zero z))) hx ht) ?_ ?_
    · intro m z' hz'
      exact hLandingObligations (m + 1) z' (.succ ht hz')
    · intro m z' hz'
      exact hCanRightAlongRun (m + 1) z' (.succ ht hz')
...
theorem shiftLocalS_of_landingObligationsAlongRun {w : List (Fin 2)}
  ...
  refine shiftLocalS_of_parts centre place entry q first
    (chainPosInv2_alongRun centre place entry q first h hx hLandingObligations hCanRightAlongRun)
...
theorem chainPosInv2_alongTrace {w : List (Fin 2)} {st : ℕ → State GalilVM} {Tc : ℕ → ℕ}
  ...
  | succ n ih =>
    intro hIndexLeTc
    exact chainPosInv2_tick_of_landingObligationsAt centre place entry q first
      (hLandingObligations n (by omega)) (hCanRightAlongTrace (n+1) hIndexLeTc) (ih (by omega))
...
theorem needBound_of_landingObligationsSansRadiusLedger {w : List (Fin 2)} (hw : 0 < w.length)
  ...
  have hinv := chainPosInv2_alongTrace centre place entry q first hPreTrace.base.pre
...
theorem marksInv_alongTrace {w : List (Fin 2)} {st : ℕ → State GalilVM} {Tc : ℕ → ℕ}
    (hPreTrace : PreTrace centre place entry q first w st Tc)
    (hMarksEntry : PalPeg.CloseoutPackRun16.H_marksEntry'
      (PofC centre place entry w) q first) :
    ∀ i, i ≤ Tc w.length →
      PalPeg.CloseoutPackRun16.MarksInv' first (st i).ctl (st i).vm :=
  fun i hIndexLeTc =>
    PalPeg.CloseoutPackRun16.marksInv'_of_run (PofC centre place entry w) q first 2048
      hMarksEntry (PalPeg.CloseoutPackRun2.steps_of_trace hPreTrace.trace i hIndexLeTc)
      (by decide : Mode.init ≠ Mode.rewind)
      (by rw [hPreTrace.start]; rfl)

#print axioms marksInv_alongTrace

/-- **着地状態の `canRight` は trace が無償で与える。**  `MatchRest.canRNext`
（源状態で 2 歩分の余裕）が偽だったのに対し、消費者が本当に要るのは
**着地状態の 1 歩分**で、それが scan 相なら `scanRightHeadCanRight_alongTrace`、
shift 相なら `shiftRightHeadCanRight_alongTrace` が持っている。
boot（`mode = init`）では guard が偽なので空虚。 -/
theorem canRightAtScanOrShift_alongTrace {w : List (Fin 2)} (hw : 0 < w.length)
    {st : ℕ → State GalilVM} {Tc : ℕ → ℕ}
    ...
    ∀ i, i ≤ Tc w.length →
      ...
      GalilScaffoldChainVerifier.canRight (st i).vm.right := by
  intro i hIndexLeTc hMode
  rcases Nat.eq_zero_or_pos (Tc w.length) with hTcZero | hTcPos
  · exfalso
    rw [hTcZero] at hIndexLeTc
    have hIndexZero : i = 0 := by omega
    subst hIndexZero
    rw [hPreTraceIMW.base.pre.start] at hMode
    rcases hMode with hm | hm <;> exact Mode.noConfusion hm
  · rcases hMode with hm | hm
    · exact scanRightHeadCanRight_alongTrace centre place entry q first hw
        hPreTraceIMW.base.pre (fun j hj => (hPreTraceIMW.packs j hj).m2) hTcPos i hIndexLeTc hm
    · exact shiftRightHeadCanRight_alongTrace centre place entry q first hw
        hPreTraceIMW.base.pre (fun j hj => (hPreTraceIMW.packs j hj).m2) i hIndexLeTc hm

#print axioms canRightAtScanOrShift_alongTrace
...
theorem radiusExactOffRewindPhase_tick_of_replay {w : List (Fin 2)} {x y : State GalilVM}
  ...
  | shift_one c s s' hm hRemPos hOne =>
    intro _ _ _
    -- `canRight center` は `shiftFrame.shiftOne` の第 1 連言（構成子が持っている）
    have hCanRightCentre : GalilScaffoldChainVerifier.canRight s.center := hOne.1.1
...
/-- **`CPack` を trace に運ぶ（1 手目から）。**  `GalilTrailRad.live_pack_trace` が
内部でやっていることを露出させただけ: boot の 1 手目の着地は `Inv`
（`GalilTrailFront.inv_of_boot_tick`）、そこから `InvL` ＋ `EntryCounters` を作って
`cpack_of_entry`、`hfloor` は `GalilInvPlus2` 経由、あとは `cpack_steps`。 -/
theorem cpack_alongTrace {w : List (Fin 2)} (hw : 0 < w.length)
    {st : ℕ → State GalilVM} {Tc : ℕ → ℕ}
    (hPreTrace : PreTrace centre place entry q first w st Tc)
    (hTcPos : 1 ≤ Tc w.length) :
    ∀ i, 1 ≤ i → i ≤ Tc w.length → PalPeg.GalilCentreLive.CPack q (st i).ctl (st i).vm := by
...
/-- **1 tick 保存。**  側入力は位置上界 2 つ（どちらもタダ）、`mode ≠ init`
（1 手目以降は定理）、そして rewind 相の `MarksInv'` ＋ `RCouple`。
`rewind_pair` は `¬ atFirst` を構成子として持つので
`two_le_left_of_marksInv'` がそのまま効く（`CentreMargin` は要らない）。 -/
theorem headsRepresent_tick {w : List (Fin 2)} (hw : 0 < w.length) {x y : State GalilVM}
    ...
    (hMarksInv : PalPeg.CloseoutPackRun16.MarksInv' first x.ctl x.vm)
    (hRCouple : PalPeg.CloseoutPackRun13.RCouple x.ctl x.vm)
  ...
  | rewind_one c s s' hm _ _ h =>
    ...
    · rw [h.2, h.1.2]; exact hCentre
    · rw [h.2, h.1.2]; exact hRight
  | rewind_pair c s s' hm hNotAtFirst _ h =>
    -- `rewind_pair` は `¬ atFirst` を構成子として持つので `MarksInv'` がそのまま効く
    have hTwoLeLeft : 2 ≤ position s.left :=
      PalPeg.CloseoutPackRun16.two_le_left_of_marksInv' hMarksInv hm hNotAtFirst
    have hTwoLeCentre : 2 ≤ position s.center := by
      obtain ⟨r, -, hLeftLeCentre, -⟩ := hRCouple hm
      exact le_trans hTwoLeLeft hLeftLeCentre
    refine ⟨?_, ?_⟩
    · rw [h.2, h.1.2]
      exact headsRepresent_leftMove hCentre.1 hCentre.2 hTwoLeCentre
    · rw [h.2, h.1.2]; exact hRight
...
theorem headsRepresent_alongTrace {w : List (Fin 2)} (hw : 0 < w.length)
  ...
  have hRCouple := rcouple_alongTrace centre place entry q first hPreTrace
  intro i
  induction i with
  ...
  | succ n ih =>
    ...
    · exact headsRepresent_tick centre place entry q first hw
        ...
        (hMarksInv n (by omega)) (hRCouple n (by omega)) (ih hnPos (by omega))

/-- **`radiusExact`（rewind 相を除く）を trace 全域で。新規入力ゼロ。**
scan 供給は `scanRightHeadCanRight_alongTrace` ＋ `represented_position`、
shift 供給は `LPackM2.shiftGeom` だけ（`canRight center` は `shiftOne` の構成子が持つ）。
init 相の `radius = 0` は `RadLedger.initZero`。
**`HeadsRepresent` に依存しない**ので `marksEntry` とは独立。 -/
theorem radiusExactOffRewindPhase_alongTrace {w : List (Fin 2)} (hw : 0 < w.length)
  ...
  -- **`ShiftGeom` だけで出る**（`HeadsRepresent` を経由しない = `marksEntry` に依存しない）:
  -- `position center = position left + rem + r` かつ `0 < position left + 2·rem` なので
  -- `0 < position center`、そこから `0 < |center.left|`（`position p = 0 ↔ |left| = 0`）。
  have hShiftSupply : ∀ j, j ≤ Tc w.length → (st j).ctl.mode = Mode.shift →
      0 < (st j).vm.center.head.left.length := by
    intro j hj hMode
    obtain ⟨rem, r, -, -, -, -, -, hLeftPos, -, hLeftNonzero, -⟩ :=
      (hLPackM2 j hj).shiftGeom hMode
    have hCentrePos : 0 < position (st j).vm.center := by omega
    by_contra hz
    have h0 : (st j).vm.center.head.left.length = 0 := by omega
    unfold position at hCentrePos
    cases hg : (st j).vm.center.gap <;> simp [hg, h0] at hCentrePos
  intro i
  induction i with
  | zero =>
    intro _
...
/-- **`EntryCounters` を trace の scan 状態で（新規入力ゼロ）。**
4 節の出どころ:

* `ScanInvariant` — `LPackM.scanGeom` / `LPackM2.scanGeomR`（両 replaying 分岐）
* `RadiusRep radius Rad` — `RadLedger.canon` ＋ `radiusExactOffRewindPhase_alongTrace`
  ＋ `ScanInvariant.rightPos`（`position right = position center + Rad`）
* `SpanRep` — `spanRepOnScanAndShift_alongTrace`
* `Canonical length` — `CPack.canon`（`cpack_alongTrace`）

これで `CloseoutMarksFree.MarksRun` の 2 半分がどちらも無償になる。 -/
theorem entryCounters_alongTrace {w : List (Fin 2)} (hw : 0 < w.length)
    {st : ℕ → State GalilVM} {Tc : ℕ → ℕ}
    (hPreTraceIMW : PalPeg.CloseoutCheckW.PreTraceIMW centre place entry q first w st Tc)
    (hTcPos : 1 ≤ Tc w.length) :
    ∀ i, 1 ≤ i → i ≤ Tc w.length → (st i).ctl.mode = Mode.scan →
      PalPeg.GalilGlueBLeaves.EntryCounters w (st i).vm := by
  have hPreTrace := hPreTraceIMW.base.pre
  have hLPackM2 : ∀ j, j ≤ Tc w.length → LPackM2 w (st j).ctl (st j).vm :=
    fun j hj => (hPreTraceIMW.packs j hj).m2
  have hRadLedger := radLedger_pt centre place entry q first hw hPreTrace
    (fun j hj => leftLive_of_lpackM (hPreTraceIMW.packs j hj).pack)
  have hRadiusExact := radiusExactOffRewindPhase_alongTrace centre place entry q first hw
    hPreTraceIMW hTcPos
  have hSpanRep := spanRepOnScanAndShift_alongTrace centre place entry q first hPreTrace
  have hCPack := cpack_alongTrace centre place entry q first hw hPreTrace hTcPos
  intro i hIndexPos hIndexLeTc hMode
  obtain ⟨Rad, hScanInv⟩ : ∃ r : ℕ,
      ScanInvariant w (position (st i).vm.center) r (st i).vm.left (st i).vm.right := by
    cases hr : (st i).ctl.replaying with
    | false => exact (hLPackM2 i hIndexLeTc).packM.scanGeom hMode hr
    | true => exact (hLPackM2 i hIndexLeTc).scanGeomR hMode hr
  refine ⟨Rad, hScanInv, ⟨(hRadLedger i hIndexLeTc).canon, ?_⟩, ?_,
    (hCPack i hIndexPos hIndexLeTc).canon⟩
  · have hExact := hRadiusExact i hIndexLeTc (by rw [hMode]; decide) (by rw [hMode]; decide)
      (by rw [hMode]; decide)
    have hRight := hScanInv.rightPos
    omega
  · exact hSpanRep i hIndexLeTc (Or.inl hMode)

#print axioms entryCounters_alongTrace
  ...
  · have hw : 0 < w.length := by
    ...
    exact (chainVerifierRepresents_step
      (chainVerifierRepresents_alongTrace centre place entry q first hw hPreTraceIMW
        hMarksInv hTcPos j hIndexLeTc) hStep).watchVer wch hy
...
theorem chainVerifierSupply_alongTrace_of_centreMargin {w : List (Fin 2)}
  ...
  exact chainVerifierSupply_alongTrace centre place entry q first hw hPreTraceIMW
    hMarksInv (by omega) i hIndexLeTc hMode
