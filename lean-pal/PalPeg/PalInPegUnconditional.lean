...
| `obligation_cycleOracleOnPackedRun` | `CycleOracleOn (ScanOnPackedRunFromInvLPS)`（run 形の cycle oracle） | n252 で `CycleOracleMC3` から切り直し。producer は `OracleReady.cycleOracleOn_of_readyLeaves`（n255／n256）。運ぶ述語は restart 無しの run（`ShapedRun.ShapedSteps`）を含み、残る葉は 4 つ: `hfresh`（fresh restart での readiness datum ＝ DP の較正）／`hchain`（`ChainReady`）／`hminv`（`MInv`）／`hfallback`（fallback＋replay、着地まで shaped）。`hready` は `ReadyTransport.readyField3_of_invLPS_shaped` ＋ `OracleReady.searchReady_of_invLPS_shaped` で定理化（n256 で `hrestart`／`hreplayStart` を消した）、shift 相は `shiftLeaf` |
