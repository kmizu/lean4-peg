...
| axiom | 内容 | 経路と残り |
|---|---|---|
| `obligation_cycleOracleOnPackedRun` | `CycleOracleOn (ScanOnPackedRunFromInvLPS) (ShapedRun.OracleTick entry)` | n268 で canonical 方針を **restart-first**（Scala 正本 `ScaffoldGalil.scala:230`）に戻した。no-restart では broken chain の fallback で Galil の移動不等式 `R ≤ 4d` が破れる（Python 正本から restart 分岐だけ除いた実行で `R=25, d=6`；restart ありでは fallback 時に chain が broken のことが無い）。producer `OracleReady.cycleOracleOn_of_readyLeaves` の葉: `hmove`（「restart guard の下」が前提）のみ。旧葉 `hshiftPeriodMinimal` は n271 で `RestartLowerRun.scanMinimal_packed` → `CanonicalChainMinimal.shiftPeriodMinimal_packed` が証明して接続済み（origin の `PackedFromBoot` を使う）。旧葉 `hrestartStage`（guard 状態での restart 着地が `Restarted ∧ StageEntry`）は n270 で `RestartCertificate.restartStage` が証明して `OracleReady` に接続済み。 |
