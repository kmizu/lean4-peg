...
/-!
# The fallback cycle as a run with a state predicate

The fallback phases (`copy`/`home`, `fpp`, `markEnd`/`choose`, `rewind`,
`replayStart`) never tick in scan mode, so the transfers of their runs to
`galilFrameS` carry `NoScan`; the cycle from the mismatching comparison to
the scan re-entry then carries any predicate that holds off scan mode and
at the two scan endpoints. The re-entry output is exported with its refresh
clause when the chosen radius is `0` (no replay).
-/
...
/-- Not in scan mode. -/
def NoScan (st : State GalilVM) : Prop := st.ctl.mode ≠ .scan
...
theorem stepsAll_transfer_generic' {σ' : Type} (L : Lens GalilVM σ') (F' : Frame σ') (G : Frame GalilVM)
    ...
    (hQ : ∀ (c : Control) (s : GalilVM), M c.mode ∨ E c.mode → Q ⟨c, s⟩)
    ...
    (n : ℕ) : ∀ {c c' : Control} {s t : GalilVM}, M c.mode ∨ E c.mode → Inv s →
      Steps (Frame.pull L F') delay n ⟨c, s⟩ ⟨c', t⟩ → StepsAll G delay Q n ⟨c, s⟩ ⟨c', t⟩ ∧ Inv t := by
  induction n with
  | zero =>
    intro c c' s t hm hi h
    cases h
    exact ⟨.zero _ (hQ c s hm), hi⟩
  | succ n ih =>
    intro c c' s t hm hi h
    cases h with
    | succ ht hr =>
      ...
      exact ⟨.succ (hQ c s hm) hg hr', hi'⟩

theorem stepsAll_transfer_fallback_S (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) (n : ℕ)
    {c c' : Control} {s t : GalilVM} (hm : c.mode = .copy ∨ c.mode = .home) (hi : ShiftIdle s)
    (h : Steps (Frame.pull fppLens (fallbackFrame (fun _ => True) (fun _ => True))) delay n ⟨c, s⟩ ⟨c', t⟩) (hend : c'.mode ≠ .scan) :
    StepsAll (galilFrameS P q first) delay NoScan n ⟨c, s⟩ ⟨c', t⟩ ∧ ShiftIdle t :=
  stepsAll_transfer_generic fppLens _ _ delay (fun m => m = .copy ∨ m = .home) (fun m => m = .fpp) ShiftIdle NoScan (fun _ _ hm' hsc => by rcases hm' with h0 | h0 <;> rw [hsc] at h0 <;> cases h0)
    ...
    (fun hm' hi' ht => tick_S_of_tick P q first delay (fallback_transfer P q first delay _ _ hm' hi' ht)
      (by rcases hm' with h | h <;> rw [h] <;> decide)) n hm hi h hend

theorem stepsAll_transfer_fpp_S (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) (n : ℕ)
    {c c' : Control} {s t : GalilVM} (hm : c.mode = .fpp) (hi : ShiftIdle s)
    (h : Steps (Frame.pull fppLens (fppFrame q first (fun _ => True) (fun _ => True))) delay n ⟨c, s⟩ ⟨c', t⟩) (hend : c'.mode ≠ .scan) :
    StepsAll (galilFrameS P q first) delay NoScan n ⟨c, s⟩ ⟨c', t⟩ ∧ ShiftIdle t :=
  stepsAll_transfer_generic fppLens _ _ delay (fun m => m = .fpp) (fun m => m = .markEnd) ShiftIdle NoScan (fun _ _ hm' hsc => by rw [hsc] at hm'; cases hm')
    ...
    (fun hm' _ ht => tick_S_of_tick P q first delay (fpp_transfer P q first delay _ _ hm' ht)
      (by rw [hm']; decide)) n hm hi h hend

theorem stepsAll_transfer_markEnd_S (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) (n : ℕ)
    {c c' : Control} {s t : GalilVM} (hm : c.mode = .markEnd) (hi : ShiftIdle s)
    (h : Steps (Frame.pull fppLens (marksFrame first (fun _ => True) (fun _ => True))) delay n ⟨c, s⟩ ⟨c', t⟩) (hend : c'.mode ≠ .scan) :
    StepsAll (galilFrameS P q first) delay NoScan n ⟨c, s⟩ ⟨c', t⟩ ∧ ShiftIdle t :=
  stepsAll_transfer_generic' fppLens _ _ delay (fun m => m = .markEnd) (fun m => m = .choose) ShiftIdle NoScan (fun _ _ hm' hsc => by rcases hm' with h0 | h0 <;> rw [hsc] at h0 <;> cases h0)
...
theorem stepsAll_transfer_rewind_S (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) (n : ℕ)
    {c c' : Control} {s t : GalilVM} (hm : c.mode = .choose ∨ c.mode = .rewind) (hi : ShiftIdle s)
    (h : Steps (Frame.pull rewindLens (rewindFrame first (fun _ => True) (fun _ => True))) delay n ⟨c, s⟩ ⟨c', t⟩) (hend : c'.mode ≠ .scan) :
    StepsAll (galilFrameS P q first) delay NoScan n ⟨c, s⟩ ⟨c', t⟩ ∧ ShiftIdle t :=
  stepsAll_transfer_generic rewindLens _ _ delay (fun m => m = .choose ∨ m = .rewind) (fun m => m = .replayStart) ShiftIdle NoScan (fun _ _ hm' hsc => by rcases hm' with h0 | h0 <;> rw [hsc] at h0 <;> cases h0)
...
theorem copy_home_start_All (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) (c : Control)
    ...
    ∃ t : FppControl.State,
      StepsAll (galilFrameS P q first) delay NoScan (2*w.length+3) ⟨c, s⟩ ⟨{c with mode := .fpp}, {s with fpp := t}⟩ ∧
  ...
  obtain ⟨hg, hi'⟩ := stepsAll_transfer_fallback_S P q first delay (2*w.length+3) (Or.inl hm) hi hst' (fun h0 => by cases h0)
...
theorem fpp_then_markEnd_All (P : Shared) (q : ℕ) (hq : 0 < q) (first : Fin 9) (delay : ℕ)
  ...
  obtain ⟨hg2, hi2⟩ := stepsAll_transfer_markEnd_S P q first delay (w.length - 1 + 1) hm1 hi1 hs2 (fun h0 => by cases h0)
...
theorem choose_then_rewind_All (P : Shared) (q : ℕ) (first : Fin 9) (h7 : first ≠ 7) (h8 : first ≠ 8)
    ...
    ∃ y : RewindVM, StepsAll (galilFrameS P q first) delay NoScan ((w.length - (2*r+1) + 1) + (2*r + 1)) ⟨c, s⟩
  ...
  obtain ⟨hg1, hi1⟩ := stepsAll_transfer_rewind_S P q first delay _ (Or.inl hm) hi hs1 (fun h0 => by cases h0)
  -- ## rewind
  ...
  obtain ⟨hg2, hi2⟩ := stepsAll_transfer_rewind_S P q first delay _ (Or.inr rfl) hi1 hs2 (fun h0 => by cases h0)
...
theorem fallback_chain_All (P : Shared) (q : ℕ) (hq : 0 < q) (first : Fin 9) (h7 : first ≠ 7) (h8 : first ≠ 8)
  ...
  have hw : 1 ≤ w.length := by
    ...
    omega
  obtain ⟨t, hg1, hmode, hprog, _, hi1⟩ := copy_home_start_All P q first delay c hm s hi old p length hc ℓ hv hs
  have hm1 : ({c with mode := .fpp} : Control).mode = .fpp := rfl
  obtain ⟨n2, y2, hg2, hden2, hhead2, _, _, _, _, hi2⟩ :=
    fpp_then_markEnd_All P q hq first delay {c with mode := .fpp} hm1 {s with fpp := t} hi1 hmode w hw hprog
  ...
  exact ⟨_, y3, stepsAll_trans hg1 (stepsAll_trans hg2 hg3), hl, hc3, hr3, hlen, hrad, hprog3, hi3⟩

theorem fallback_to_scan_All (onLetter leftFirst guard : GalilVM → Prop) (bs bf rs : GalilVM → GalilVM → Prop) (centre : GalilVM → Fin 3) (place : GalilVM → GalilScaffoldPlace.Place) (entry : ℕ) (q : ℕ) (hq : 0 < q) (first : Fin 9)
  ...
  obtain ⟨n, y, hg, hl, hc', hr, hlen, hrad, hprog, hi'⟩ :=
    fallback_chain_All (galilShared onLetter leftFirst guard bs bf rs centre place entry) q hq first h7 h8 delay c hm s hi old p length hc ℓ hv hs hne heven
  ...
  refine ⟨n, o, t, fun Q hQ hQend => stepsAll_trans (stepsAll_mono (fun st h0 => hQ st h0) hg)
      (.succ (hQ _ (fun h0 => by cases h0)) htS (.zero _ hQend)), ?_, ?_, ?_, ?_, hrad', hlen', hw, ?_, ?_, ?_,
...
theorem scan_fallback_cycle_All (onLetter leftFirst : GalilVM → Prop) (rs : GalilVM → GalilVM → Prop)
...
#print axioms fallback_to_scan_All
#print axioms scan_fallback_cycle_All
