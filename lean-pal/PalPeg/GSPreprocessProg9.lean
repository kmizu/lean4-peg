...
def RESET_SIGNED (k : ℕ) : Prog A9 Cond9 :=
  .seq (REWIND_SIGNED k) (.seq MAX_ONE (SHIFT_SIGNED k))

theorem RESET_SIGNED_spec {startSym : Fin sc} {x : List (Fin sc)}
    ...
    ∃ L D' g', ExecA Terminal blank endSym mark (RESET_SIGNED k) ts L ∧
      ...
      L.length ≤ 15 * q + 7 + (3 * k + 7) * shiftNoPeriod q k := by
  obtain ⟨L1, D1, g1, hx1, he1, ha1, hb1, hc1⟩ :=
    REWIND_SIGNED_spec (Terminal := Terminal) hmark k q s (s + p) D 0 p F S R
...
/-- info: 'PalPeg.GSPreProg.RESET_SIGNED_spec' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms RESET_SIGNED_spec
