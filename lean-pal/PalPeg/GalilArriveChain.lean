...
theorem watchBreak_arrive (a : Fin 2) {w : GalilScaffoldChainWatch.State} (h : WatchBreak w) :
    WatchBreak (arriveW a w) := by
  obtain ⟨hp, hc, b, hs, hr⟩ := h
  refine ⟨hp, canRight_arrive a _, b, hs, ?_⟩
  show GalilScaffoldInputHead.read (GalilScaffoldChainVerifier.right (arrivePH a w.machine.verifier)) ≠ some b
  rw [right_arrive a _ hc, read_arrive]; exact hr
...
theorem chainStep_arrive (a : Fin 2) {x y : ChainVM} (h : ChainStep x y) :
  ...
  | watchBreak w hb =>
    have := ChainStep.watchBreak (arriveW a w) (watchBreak_arrive a hb)
    have h' : (arriveW a w).machine.verifier = arrivePH a w.machine.verifier := rfl
    rw [h', right_arrive a _ hb.2.1] at this
    exact this
