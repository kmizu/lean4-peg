...
/-! ## 10. Carrying the search stage through the replay

`ReplayBudget` is only true of the found ticks a replay *reaches*: the bound
`k ≤ 2n` behind its clauses is a fact about the search history.  The datum that
records the history is `ReplayStageD` — the found tick ends a chain-idle
segment out of a `Restarted`/`StageEntry` state with a full clock — and it is
carried, not assumed: it is free at the replay entry, extends along `countR` and
`matchIdleR`, and is *re-established* at a mid-replay restart, because at the
break the chain has consumed `R - C ≥ 4h` places, so `last ≥ 3h` and
`3·(R+1-C) ≤ 5·last` (`chainW_break'`, `stage_arith`).  The restart must keep
`length` and set `lower := last` for the restarted state to be `Restarted`;
`RestartShapeL` records that (true of `restartVM`). -/
...
/-- The found tick sits inside the replay run: it ends a chain-idle segment out
of a restarted state with a full clock.  (`GalilFoundStage.ReplayStage` with the
clock `delay` in place of `2048`; at `delay = 2048` the two are definitionally
equal.) -/
def ReplayStageD (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ)
    (c : Control) (s : GalilVM) : Prop :=
  ∃ (r : GalilVM) (Rad : ℕ) (last : Counter) (es : List Bool) (c0 : Control),
    Restarted raw r Rad last ∧ StageEntry Rad last ∧ c0.clock = delay ∧
      WatchSegE P q first delay es c0 r c s

/-- `ReplayBudget` restricted to the found ticks carrying `ReplayStageD`
(`GalilFoundStage.ReplayBudgetR`, definitionally, at `delay = 2048`). -/
def ReplayBudgetRD (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) : Prop :=
  ∀ (c : Control) (s : GalilVM) (a : Bool) (vq : SearchVM) (k m n : ℕ) (xs : List (Fin 3))
    (b : Fin 3),
    c.mode = .scan → c.replaying = true → s.chain = .idle → SearchReady (searchLens.get s) →
    searchEffect P a s vq → vq.search.mode = .found → MInv raw c s →
    ...
    AnswerAhead (vq.dp.config.tapes 11) n → 0 < n → PlaceAhead (P.place s) n →
    (GalilScaffoldPlace.stream (P.place s)).tail.take n = xs ++ [b] →
    ReplayStageD raw P q first delay c s →
    ∀ j, position s.center + 1 ≤ j → j ≤ position s.right + m →
      Mispredicted raw (P.centre s) b xs (position s.center + 1) j →
      position s.center + 4 * n < j ∧ position s.right + 2 ≤ j ∧
        2 * n + 3 + k ≤ (j - (position s.right + 2)) * (delay - 1)

/-- `RestartShape` plus what `Restarted` needs of the restarted state: the
lower bound is the chain's `last` and the length counter is kept. -/
def RestartShapeL (P : Shared) : Prop :=
  ∀ (s : GalilVM) (w : WState), s.chain = .broken w → Restartable w →
    ∃ s', P.restart s s' ∧ s'.chain = .idle ∧ s'.left = s.left ∧ s'.right = s.right ∧
      s'.center = s.center ∧ s'.replay = s.replay ∧ s'.radius = s.radius ∧
      s'.remaining = s.remaining ∧
      s'.search = GalilScaffoldSearchFinish.begin w.machine.control.last s.radius ∧
      s'.lower = w.machine.control.last ∧ s'.length = s.length

/-- `Entry` with the stage datum, the canonical length counter, and — when no
comparison is left, so that the entry is itself the landing — the output
soundness at the landing. -/
def EntryS (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay B : ℕ)
    (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  Entry raw delay B c s k r ∧ ReplayStageD raw P q first delay c s ∧ Canonical s.length ∧
    (r = 0 → OutputRel raw c s)

/-- `LiveEnd` whose break branch carries the stage datum of the restarted state. -/
def LiveEndS (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay B : ℕ)
    (cc b : Fin 3) (xs : List (Fin 3)) (c : Control) (s : GalilVM) (k m : ℕ) : Prop :=
  (∃ (es : List Bool) (c' : Control) (t : GalilVM),
      ReplayChainSeg P q first delay es c s c' t ∧ es.count true = m ∧
      c'.mode = .scan ∧ c'.clock = delay ∧ c'.replaying = false ∧ t.replay = reset ∧
      ChainW raw (position t.center) B (position t.right) (budOf delay B c' t) false cc b xs t.chain ∧
  ...
  (∃ (es : List Bool) (c' : Control) (t : GalilVM) (m' : ℕ),
      ...
      t.center = s.center ∧ Frontier t ∧ t.remaining = s.remaining ∧ Bnd B t ∧
      (P.onLetter = onLetterVM raw → P.leftFirst = leftFirstVM → m' = 0 → OutputRel raw c' t) ∧
      ReplayStageD raw P q first delay c' t ∧ Canonical t.length)
...
/-- Any chain-idle stretch extends the stage segment. -/
theorem replayStageD_trans {c c' : Control} {s s' : GalilVM} {es : List Bool}
    (hst : ReplayStageD raw P q first delay c s) (hseg : WatchSegE P q first delay es c s c' s') :
    ReplayStageD raw P q first delay c' s' := by
  obtain ⟨r, Rad, last, es0, c0, hR, hSt, hcl, hseg0⟩ := hst
  exact ⟨r, Rad, last, es0 ++ es, c0, hR, hSt, hcl, watchSegE_trans P q first delay hseg0 hseg⟩

theorem restartShape_of_L (h : RestartShapeL P) : RestartShape P := by
  intro s w hs hw
  obtain ⟨s', h1, h2, h3, h4, h5, h6, h7, h8, h9, -, -⟩ := h s w hs hw
  exact ⟨s', h1, h2, h3, h4, h5, h6, h7, h8, h9⟩

theorem restartShapeL_of_restartVM (entry : ℕ) (h : P.restart = restartVM entry) :
    RestartShapeL P := by
  intro s w hs hw
  refine ⟨restartState entry s w, ?_, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl⟩
  rw [h]
  exact ⟨w, hs, hw.margin, hw.last, hw.lag, rfl⟩

theorem restartShapeL_sharedC (onLetter leftFirst : GalilVM → Prop) (centre : GalilVM → Fin 3)
    (place : GalilVM → GalilScaffoldPlace.Place) (entry : ℕ) :
    RestartShapeL (sharedC onLetter leftFirst centre place entry) :=
  restartShapeL_of_restartVM entry rfl

/-- **The stage inequality from the consumed places.**  After `k ≥ 4h` consumed
places the chain's `last` is at least `(⌊k/h⌋ - 1)·h`, and then
`3·(k+1) ≤ 5·last`. -/
theorem stage_arith {k h : ℕ} {last : ℤ} (hh : 1 ≤ h) (hk : 4 * h ≤ k)
    (hlast : ∀ q : ℕ, (q + 1) * h ≤ k → ((q * h : ℕ) : ℤ) ≤ last) :
    3 * ((k + 1 : ℕ) : ℤ) ≤ 5 * last := by
  have hd4 : 4 ≤ k / h := (Nat.le_div_iff_mul_le (by omega)).2 hk
  obtain ⟨d, hd⟩ : ∃ d, k / h = d + 1 := ⟨k / h - 1, by omega⟩
  have hd3 : 3 ≤ d := by omega
  have hdm := Nat.div_add_mod k h
  have hml := Nat.mod_lt k (show h > 0 by omega)
  rw [hd, Nat.mul_add, Nat.mul_one] at hdm
  generalize k % h = rr at hdm hml
  have hle : (d + 1) * h ≤ k := by rw [Nat.add_mul, Nat.one_mul, Nat.mul_comm d h]; omega
  have h1 := hlast d hle
  rw [Nat.mul_comm d h] at h1
  have hm3 : h * 3 ≤ h * d := Nat.mul_le_mul_left h hd3
  generalize h * d = hd' at hdm h1 hm3
  omega

/-- The length counter stays canonical along a live-chain replay segment. -/
theorem replayChainSeg_canonical {es : List Bool} {c c' : Control} {s t : GalilVM}
    (h : ReplayChainSeg P q first delay es c s c' t) :
    Canonical s.length → Canonical t.length := by
  induction h with
  | stop c s => exact id
  | countC c s s' hm hr hc hne hb _ ih =>
    intro hcan
    have hl : s'.length = s.length := (backgroundS_fields P q first hb).2.2.2.2.2.2.1
    exact ih (by rw [hl]; exact hcan)
  | matchC c s vs vq o hm hr hc ha hne hcmp hmt hq ho _ ih =>
    intro hcan
    exact ih (inc_canonical _ (inc_canonical _ hcan))

theorem cmpU_length (s : GalilVM) (z : ChainVM) : (cmpU s z).length = inc (inc s.length) := rfl
...
variable {raw : List (Fin 2)} {C E : ℕ} {cc b : Fin 3} {xs : List (Fin 3)}

/-- `chainW_break` with the stage inequality: at the break the chain has
consumed `R - C ≥ 4h` places from `ready`, so `sweep_last_lower_sharp` gives
`last ≥ (⌊(R-C)/h⌋ - 1)·h` and `3·(R+1-C) ≤ 5·last`. -/
theorem chainW_break' {R : ℕ} {y : ChainVM} (hy : ChainW raw C E R 0 true cc b xs y)
    (hB : R + 1 < (encoded raw).length)
    (hmis : Mispredicted raw cc b xs (C+1) (R+1)) (h4 : C + 4 * (xs.length + 1) ≤ R) :
    ∃ w', ChainMatched y (.broken w') ∧ Restartable w' ∧
      3 * ((R + 1 - C : ℕ) : ℤ) ≤ 5 * value w'.machine.control.last := by
  cases y with
  | idle => exact hy.elim
  | broken w => exact hy.elim
  | copy t h p v lag margin ver =>
  ...
  | watch w =>
    ...
    have hctl : (consume w.machine).control.last = w.machine.control.last := by
      ...
      rw [GalilScaffoldChainConsume.mismatch _ a _ hsym hread]
    obtain ⟨hL1, hL2, hL3⟩ := coreX_last hc (by rw [hvpos]; omega)
    have hst : 3 * ((R + 1 - C : ℕ) : ℤ) ≤ 5 * value w.machine.control.last := by
      obtain ⟨-, -, -, pre, hctl', hbr', hidx⟩ := hc
      have hpl : pre.length = R - C := by omega
      rw [hctl', show R + 1 - C = R - C + 1 from by omega]
      refine stage_arith (h := xs.length + 1) (by omega) (by omega) (fun q' hq' => ?_)
      exact PalPeg.GalilLastLowerBreak.sweep_last_lower_sharp cc b xs pre q' hbr'
        (by rw [hpl]; exact hq')
    refine ⟨_, .breaks _ _ hbr, ⟨hz, ?_, ?_, ?_, ?_⟩, ?_⟩
    · show negative (inc w.margin) = false
      have hc' := inc_canonical _ hcan
      cases hn : negative (inc w.margin) with
      | false => rfl
    ...
    · show 0 ≤ value (consume w.machine).control.last
      rw [hctl]; exact hL3
    · show 3 * _ ≤ 5 * value (consume w.machine).control.last
      rw [hctl]; exact hst
...
/-- **The break at the window's end, the restart, and the re-entry of the
stage.**  `chain_breakW` with the restarted state shown to be
`Restarted raw u (k+1) last` with `StageEntry (k+1) last`. -/
theorem chain_breakWS (hex : ∀ s, P.replayExhausted s = zero s.replay) (hrs : RestartShapeL P)
    (hBl : B < (encoded raw).length)
    (c : Control) (s : GalilVM) (k m : ℕ)
    (hm : c.mode = .scan) (hr : c.replaying = true) (hc : c.clock = 1)
    (hpart : ChainW raw (position s.center) E (position s.right) (budOf delay E c s) true cc b xs s.chain)
    (hRE : position s.right = E) (hEB : E < B)
    (hmis : Mispredicted raw cc b xs (position s.center + 1) (E + 1))
    (h4 : position s.center + 4 * (xs.length + 1) ≤ E)
    (hbd : Bnd B s) (hM : MInv raw c s) (hi : ScanInvariant raw (position s.center) k s.left s.right)
    (hrp : s.replay = ofNat (m+1)) (hfr : Frontier s) (hrad : RadiusRep s.radius k)
    (hcen : GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none)
    (hlen : Canonical s.length) :
    ∃ (c1 : Control) (u : GalilVM),
      ReplayChainSeg2 P q first delay [true] c s c1 u ∧
      c1.mode = .scan ∧ c1.clock = delay ∧ c1.replaying = decide (0 < m) ∧
      u.chain = .idle ∧ GalilBranchInvariants2.SearchReady (searchLens.get u) ∧
      ...
      u.center = s.center ∧ Frontier u ∧ u.remaining = s.remaining ∧
      RadiusRep u.radius (k+1) ∧ Bnd B u ∧
      (P.onLetter = onLetterVM raw → P.leftFirst = leftFirstVM → OutputRel raw c1 u) ∧
      ReplayStageD raw P q first delay c1 u ∧ Canonical u.length := by
  have hne : s.chain ≠ .idle := chainW_ne_idle hpart
  have hbud : budOf delay E c s = 0 + 1 := by
    unfold budOf; rw [hc, hRE]; simp
  rw [hbud] at hpart
  obtain ⟨y, hstep, hy⟩ := chainW_step hpart (by omega) (by omega)
  rw [hRE] at hy
  obtain ⟨w', hmat, hw', hst3⟩ := chainW_break' hy (by omega) hmis h4
  obtain ⟨hav, hcmp, hmt, hq, ho, hflag, hchu, hMu, hiu, hurep, hposu, hCu, hfru, hremu, hradu, hbdu,
      hout⟩ :=
    forced_compare raw P q first delay B hex c s k m (.broken w') hr hne hbd hM hi hrp hfr hrad
      (fun _ => ⟨y, hstep, hmat⟩)
  obtain ⟨s', hb', hidle', hl', hr', hC', hrep', hrad', hrem', hsearch', hlow', hlen'⟩ :=
    hrs (cmpU s (.broken w')) w' hchu hw'
  have hRC : position s.right = position s.center + k := hi.rightPos
  have hclen : Canonical s'.length := by
    rw [hlen', cmpU_length]; exact inc_canonical _ (inc_canonical _ hlen)
  have hi' : ScanInvariant raw (position s'.center) (k+1) s'.left s'.right := by
    rw [hl', hr', hC']; exact hiu
  have hrad'' : RadiusRep s'.radius (k+1) := by rw [hrad']; exact hradu
  have hRst : Restarted raw s' (k+1) w'.machine.control.last :=
    ⟨hidle', by rw [hC', hCu]; exact hcen.1, by rw [hC', hCu]; exact hcen.2, hi', hrad'', hclen,
      by rw [hsearch', hrad'], hlow', hw'.lastCanonical, hw'.lastNonneg⟩
  have hSt : StageEntry (k+1) w'.machine.control.last := by
    intro k' hk'
    rw [hk'] at hst3
    omega
  refine ⟨{cmpC P delay c s (.broken w') with clock := delay}, s',
    .breakC c s (cmpVS s (.broken w')) (searchLens.get s) (cmpO P c s (.broken w')) w' hm hr hc hav hne
      hcmp hmt rfl hq ho (.restartC (cmpC P delay c s (.broken w')) (cmpU s (.broken w')) s' hm hb'
        hl' hr' hC' hrep' (.stop _ _)),
    hm, rfl, hflag, hidle', ?_, ?_, hi', by rw [hrep', hurep], by rw [hr', hposu], by rw [hC', hCu],
    frontier_congr hr' hrep' hfru, by rw [hrem', hremu], hrad'',
    bnd_congr hr' hrep' hbdu, ?_, ⟨s', k+1, w'.machine.control.last, [], _, hRst, hSt, rfl, .stop _ _⟩,
    hclen⟩
  · apply searchReady_of_begin (last := w'.machine.control.last)
    rw [hsearch', hrad']
  · exact minv_same rfl hr' hC' hrep' hMu
...
/-- **The live chain along the rest of the replay**, carrying the stage into
the break branch. -/
theorem chain_runWS (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hrs : RestartShapeL P) (hBl : B < (encoded raw).length) (hEB : E ≤ B)
    (hlimF : lim = false → E = B) (hlimT : lim = true → E < B) :
    ∀ (m : ℕ) (c : Control) (s : GalilVM) (k : ℕ),
      c.mode = .scan → 1 ≤ c.clock → c.replaying = decide (0 < m) → (m = 0 → c.clock = delay) →
      s.replay = ofNat m → Bnd B s →
      ...
      position s.right ≤ E →
      (lim = true → Mispredicted raw cc b xs (position s.center + 1) (E + 1) ∧
        position s.center + 4 * (xs.length + 1) ≤ E) →
      MInv raw c s → ScanInvariant raw (position s.center) k s.left s.right → Frontier s →
      RadiusRep s.radius k →
      (GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) →
      Canonical s.length →
      LiveEndS raw P q first delay B cc b xs c s k m := by
  intro m
  induction m with
  | zero =>
    intro c s k hm hclk hrp hc0 hrep hbd hpart hRE hbrk hM hi hfr hrad _ _
    have hRB : position s.right = B := by simpa using hbd 0 hrep
    cases lim with
    | true => exact absurd (hlimT rfl) (by omega)
    | false =>
      ...
      exact Or.inl ⟨[], c, s, .stop _ _, rfl, hm, hc0 rfl, by simpa using hrp, hrep, hpart, hRB, hM,
        by simpa using hi, by simp, rfl, hfr, rfl⟩
  | succ n ih =>
    intro c s k hm hclk hrp hc0 hrep hbd hpart hRE hbrk hM hi hfr hrad hcen hcan
    have hrt : c.replaying = true := by rw [hrp]; simp
    obtain ⟨j, hj⟩ : ∃ j, c.clock = j + 1 := ⟨c.clock - 1, by omega⟩
    have hElen : E < (encoded raw).length := by omega
    obtain ⟨es1, s1, hseg1, hcnt1, hp1, hl1, hr1, hC1, hrep1, hrem1, hrad1⟩ :=
    ...
    have hrad1' : RadiusRep s1.radius k := by rw [hrad1]; exact hrad
    have hrep1' : s1.replay = ofNat (n+1) := by rw [hrep1, hrep]
    have hcen1 : GalilScaffoldInputTrace.Represents s1.center.head raw ∧ s1.center.head.focus ≠ none := by
      rw [hC1]; exact hcen
    have hcan1 : Canonical s1.length := replayChainSeg_canonical hseg1 hcan
    by_cases hlt : position s1.right + 1 ≤ E
    · obtain ⟨c2, u, hseg2, hm2, hc2, hr2, hp2, hM2, hi2, hrep2, hpos2, hC2, hfr2, hrem2, hrad2, hbd2⟩ :=
        chain_compareW raw P q first delay B E lim cc b xs hex hd hElen {c with clock := 1} s1 k n hm hrt rfl
          hp1 hlt hbd1 hM1 hi1 hrep1' hfr1 hrad1'
      have hbrk2 : lim = true → Mispredicted raw cc b xs (position u.center + 1) (E + 1) ∧
          position u.center + 4 * (xs.length + 1) ≤ E := by
        rw [hC2, hC1]; exact hbrk
      have hcen2 : GalilScaffoldInputTrace.Represents u.center.head raw ∧ u.center.head.focus ≠ none := by
        rw [hC2]; exact hcen1
      have hcan2 : Canonical u.length := replayChainSeg_canonical hseg2 hcan1
      rcases ih c2 u (k+1) hm2 (by rw [hc2]; exact hd) hr2 (fun _ => hc2) hrep2 hbd2 hp2 (by omega) hbrk2
          hM2 hi2 hfr2 hrad2 hcen2 hcan2 with hA | hB'
      · obtain ⟨es3, c3, t3, hseg3, hcnt3, hm3, hc3, hr3, hrep3, hp3, hB3, hM3, hi3, hpos3, hC3, hfr3,
          hrem3⟩ := hA
        refine Or.inl ⟨es1 ++ ([true] ++ es3), c3, t3,
          replayChainSeg_trans P q first delay hseg1 (replayChainSeg_trans P q first delay hseg2 hseg3),
        ...
        · rw [hrem3, hrem2, hrem1]
      · obtain ⟨es3, c3, t3, m', hseg3, hlt3, hm3, hc3, hr3, hrep3, hidle3, hsr3, hM3, hi3, hrad3, hpos3,
          hC3, hfr3, hrem3, hbd3, hout3, hst3, hcan3⟩ := hB'
        refine Or.inr ⟨es1 ++ ([true] ++ es3), c3, t3, m',
          replayChainSeg2_trans (replayChainSeg2_of_seg hseg1)
            (replayChainSeg2_trans (replayChainSeg2_of_seg hseg2) hseg3),
          by omega, hm3, hc3, hr3, hrep3, hidle3, hsr3, hM3, ?_, ?_, ?_, ?_, hfr3, ?_, hbd3, hout3,
    ...
    · have hRE1 : position s1.right = E := by rw [hr1] at hlt ⊢; omega
      ...
      | true =>
        obtain ⟨hmis, h4⟩ := hbrk rfl
        obtain ⟨c2, u, hseg2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2, hrem2, hrad2,
            hbd2, hout2, hst2, hcan2⟩ :=
          chain_breakWS raw P q first delay B E cc b xs hex hrs hBl {c with clock := 1} s1 k n hm hrt rfl
            hp1 hRE1 (hlimT rfl) (by rw [hC1]; exact hmis) (by rw [hC1]; exact h4) hbd1 hM1 hi1 hrep1'
            hfr1 hrad1' hcen1 hcan1
        refine Or.inr ⟨es1 ++ [true], c2, u, n,
          replayChainSeg2_trans (replayChainSeg2_of_seg hseg1) hseg2, by omega, hm2, hc2, hr2, hrep2,
          hidle2, hsr2, hM2, ?_, ?_, ?_, ?_, hfr2, ?_, hbd2, fun hP hP' _ => hout2 hP hP', hst2, hcan2⟩
        · rw [show k + (n + 1 - n) = k + 1 from by omega]; exact hi2
        · rw [show k + (n + 1 - n) = k + 1 from by omega]; exact hrad2
        · rw [hpos2, hr1]; omega
        · rw [hC2, hC1]
...
/-! ## 11. Landing shapes with the output soundness, and the stage construction -/
...
/-- The idle landing with its output soundness. -/
def IdleEndO (s : GalilVM) (k r : ℕ) (c' : Control) (t : GalilVM) : Prop :=
  IdleEnd raw delay s k r c' t ∧ OutputRel raw c' t

/-- (i) with the output soundness at the landing. -/
def QuietEndS (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  ∃ (es : List Bool) (c' : Control) (t : GalilVM),
    WatchSegE P q first delay es c s c' t ∧
    (SoundScanNR raw ⟨c', t⟩ →
      StepsAll (galilFrameS P q first) delay (SoundScanNR raw) es.length ⟨c, s⟩ ⟨c', t⟩) ∧
    es.length = r * delay ∧ es.count true = r ∧ IdleEndO raw delay s k r c' t

/-- (iii) with the output soundness at the landing. -/
def RestartEndS (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  BrokeAndRestarted raw P q first delay c s ∧
  ∃ (n : ℕ) (c' : Control) (t : GalilVM),
    (SoundScanNR raw ⟨c', t⟩ →
      StepsAll (galilFrameS P q first) delay (SoundScanNR raw) n ⟨c, s⟩ ⟨c', t⟩) ∧
    IdleEndO raw delay s k r c' t

def BusyS (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  ChainEnd raw P q first delay B c s k r ∨ RestartEndS raw P q first delay c s k r

def Result3S (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  QuietEndS raw P q first delay c s k r ∨ BusyS raw P q first delay B c s k r

variable {raw P q first delay B}

theorem idleEndO_shift {s s1 : GalilVM} {k r k1 r1 : ℕ} {c' : Control} {t : GalilVM}
    (hk : k1 + r1 = k + r) (hp : position s1.right + r1 = position s.right + r)
    (hC : s1.center = s.center) (hrem : s1.remaining = s.remaining)
    (h : IdleEndO raw delay s1 k1 r1 c' t) : IdleEndO raw delay s k r c' t :=
  ⟨idleEnd_shift hk hp hC hrem h.1, h.2⟩

theorem busyS_prepend {c c1 : Control} {s s1 : GalilVM} {k r k1 r1 n0 : ℕ}
    (hst : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) n0 ⟨c, s⟩ ⟨c1, s1⟩)
    (hk : k1 + r1 = k + r) (hp : position s1.right + r1 = position s.right + r)
    (hC : s1.center = s.center) (hrem : s1.remaining = s.remaining)
    (h : BusyS raw P q first delay B c1 s1 k1 r1) : BusyS raw P q first delay B c s k r := by
  rcases h with hA | hB'
  · obtain ⟨n1, c2, s2, c3, s3, es, c', t, n, cc, b, xs, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11,
      h12, h13, h14, h15, h16, h17, h18, h19, h20, h21⟩ := hA
    exact Or.inl ⟨n0 + n1, c2, s2, c3, s3, es, c', t, n0 + n, cc, b, xs, stepsAll_trans hst h1, h2, h3,
  ...
  · obtain ⟨⟨n1, ca, sa, es, cb, sb, w1, w2, w3, w4⟩, n, c', t, hrun, hend⟩ := hB'
    exact Or.inr ⟨⟨n0 + n1, ca, sa, es, cb, sb, stepsAll_trans hst w1, w2, w3, w4⟩, n0 + n, c', t,
      fun h => stepsAll_trans hst (hrun h), idleEndO_shift hk hp hC hrem hend⟩

theorem resultS_prepend_restart {c c1 : Control} {s s1 : GalilVM} {k r k1 r1 n0 : ℕ}
    (hst : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) n0 ⟨c, s⟩ ⟨c1, s1⟩)
    (hw : BrokeAndRestarted raw P q first delay c s)
    (hk : k1 + r1 = k + r) (hp : position s1.right + r1 = position s.right + r)
    (hC : s1.center = s.center) (hrem : s1.remaining = s.remaining)
    (h : Result3S raw P q first delay B c1 s1 k1 r1) : BusyS raw P q first delay B c s k r := by
  rcases h with hQ | hBusy
  · obtain ⟨es, c', t, -, hrun, -, -, hend⟩ := hQ
    exact Or.inr ⟨hw, n0 + es.length, c', t, fun h => stepsAll_trans hst (hrun h),
      idleEndO_shift hk hp hC hrem hend⟩
  · exact busyS_prepend hst hk hp hC hrem hBusy

theorem resultS_prepend_quiet {c c1 : Control} {s s1 : GalilVM} {k r k1 r1 : ℕ} {es0 : List Bool}
    (hseg : WatchSegE P q first delay es0 c s c1 s1)
    (hst : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) es0.length ⟨c, s⟩ ⟨c1, s1⟩)
    (hk : k1 + r1 = k + r) (hp : position s1.right + r1 = position s.right + r)
    (hC : s1.center = s.center) (hrem : s1.remaining = s.remaining)
    (hlen : es0.length + r1 * delay = r * delay) (hcnt : es0.count true + r1 = r)
    (h : Result3S raw P q first delay B c1 s1 k1 r1) : Result3S raw P q first delay B c s k r := by
  rcases h with hQ | hBusy
  · obtain ⟨es, c', t, hseg', hrun, hlen', hcnt', hend⟩ := hQ
    refine Or.inl ⟨es0 ++ es, c', t, watchSegE_trans P q first delay hseg hseg', fun h => ?_, ?_, ?_,
      idleEndO_shift hk hp hC hrem hend⟩
    ...
    · rw [List.length_append, hlen']; omega
    · rw [List.count_append, hcnt']; omega
  · exact Or.inr (busyS_prepend hst hk hp hC hrem hBusy)

end StageEnds

section StageConstruct
...
/-- The window at a found search inside a replay, from the restricted budget:
`window_of_found` with the mispredicted place strictly beyond `C + 4h`. -/
theorem window_of_foundS (hbudget : ReplayBudgetRD raw P q first delay)
    (c : Control) (s : GalilVM) (a : Bool) (vq : SearchVM) (k m n : ℕ) (xs : List (Fin 3)) (b : Fin 3)
    (hm : c.mode = .scan) (hr : c.replaying = true) (hidle : s.chain = .idle)
    (hsr : SearchReady (searchLens.get s)) (hq : searchEffect P a s vq)
    (hf : vq.search.mode = .found) (hM : MInv raw c s)
    ...
    (ha : AnswerAhead (vq.dp.config.tapes 11) n) (hn : 0 < n) (hpl : PlaceAhead (P.place s) n)
    (hsplit : (GalilScaffoldPlace.stream (P.place s)).tail.take n = xs ++ [b])
    (hst : ReplayStageD raw P q first delay c s) :
    ∃ (E : ℕ) (lim : Bool),
      E ≤ position s.right + m ∧ (lim = false → E = position s.right + m) ∧
      (lim = true → E < position s.right + m) ∧ position s.right + 1 ≤ E ∧
      BlockOn raw (P.centre s) b xs (position s.center + 1) E ∧
  ...
  by_cases hEB : E < position s.right + m
  · obtain ⟨hj1, hj2, hj3⟩ := hbudget c s a vq k m n xs b hm hr hidle hsr hq hf hM hi hrad hrep hm0 hfr
      ha hn hpl hsplit hst (E + 1) (by omega) (by omega) (h4 hEB)
    refine ⟨E, true, h2, fun h => absurd h (by decide), fun _ => hEB, by omega, h3,
      fun _ => ⟨h4 hEB, by omega, ?_⟩⟩
    have hmono : (E + 1 - (position s.right + 2)) * (delay - 1) ≤ (E - position s.right) * (delay - 1) :=
      Nat.mul_le_mul_right _ (by omega)
...
/-- **From a found tick to a busy landing**, with the stage re-entered after a
break. -/
theorem found_resultS (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    {c c2 : Control} {s s2 : GalilVM} {k r r0 k2 m2 : ℕ} {cc b : Fin 3} {xs : List (Fin 3)}
    (ih : ∀ m' < r0, ∀ (c : Control) (s : GalilVM) (k : ℕ), EntryS raw P q first delay B c s k m' →
    ...
    (hQ2 : SoundScanNR raw ⟨c2, s2⟩) (hne2 : s2.chain ≠ .idle)
    (hi2 : ScanInvariant raw (position s2.center) k2 s2.left s2.right)
    (hlive : LiveEndS raw P q first delay B cc b xs c2 s2 k2 m2)
    (hk : k2 + m2 = k + r) (hp : position s2.right + m2 = position s.right + r)
    (hC : s2.center = s.center) (hrem : s2.remaining = s.remaining) (hmr : m2 ≤ r) (hrr0 : r ≤ r0)
    (hcen : GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) :
    BusyS raw P q first delay B c s k r := by
  rcases hlive with hA | hB'
  · obtain ⟨es, c', t, hseg, -, hm', hc', hr', hrep', hpart', hBt, hM', hi', hpos', hC', hfr', hrem'⟩ :=
      hA
    obtain ⟨n, hst⟩ := replayChainSeg_stepsAll raw P hP hP' q first delay hseg (position s2.center) k2
    ...
    exact Or.inl ⟨0, c, s, c2, s2, es, c', t, 1 + n, cc, b, xs,
      ...
      by rw [hrem', hrem]⟩
  · obtain ⟨es, c', t, m', hseg, hlt, hm', hc', hr', hrep', hidle', hsr', hM', hi', hrad', hpos', hC',
      hfr', hrem', hbd', hout', hstage, hcanl⟩ := hB'
    obtain ⟨n, hst⟩ := replayChainSeg2_stepsAll raw hP hP' hseg (position s2.center) k2 hi2 hQ2
    have hres := ih m' (by omega) c' t (k2 + (m2 - m'))
      ⟨⟨hm', hc', hr', hrep', hidle', hsr', hM', hi', hfr', hrad', hbd',
        by rw [hC', hC]; exact hcen.1, by rw [hC', hC]; exact hcen.2⟩, hstage, hcanl,
        fun h0 => hout' hP hP' h0⟩
    exact resultS_prepend_restart (stepsAll_trans hst1 hst)
      ⟨1, c2, s2, es, c', t, hst1, hseg, hne2, hidle'⟩ (by omega) (by omega) (by rw [hC', hC])
      (by rw [hrem', hrem]) hres

/-- The countdown of an idle replay stretch, carrying the stage. -/
theorem idle_countdown3S (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hpres : ∀ (s : GalilVM) (a : Bool) (v : SearchVM),
      SearchReady (searchLens.get s) → searchEffect P a s v → SearchReady v)
    (hshape : GalilWatchOkInst.StartShape P) (hbudget : ReplayBudgetRD raw P q first delay)
...
theorem idle_compare3S (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hpres : ∀ (s : GalilVM) (a : Bool) (v : SearchVM),
...
theorem replay_construct3S (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hpres : ∀ (s : GalilVM) (a : Bool) (v : SearchVM),
  ...
  | _ r ih =>
    ...
    | succ n =>
      have hrt : c.replaying = true := by rw [hrp]; simp
      rcases idle_countdown3S (q := q) (first := first) hP hP' hex hd hsearch hpres hshape hbudget hrs
          hBl ih (delay - 1) c s k n le_rfl hm hrt (by omega) hidle hsr hrep hM hi hfr hrad hbd
      ...
      · obtain ⟨c1, t1, hseg1, hstp1, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
        ...
        have hi1 : ScanInvariant raw (position t1.center) k t1.left t1.right := by
          rw [hl1, hrr1, hC1]; exact hi
        have hfr1 : Frontier t1 := frontier_congr hrr1 hrp1 hfr
        rcases idle_compare3S (q := q) (first := first) hP hP' hex hd hsearch hpres hshape hbudget hrs
            hBl ih c1 t1 k n le_rfl hm1' hr1' hc1 hidle1 hsr1 hM1 hi1 (by rw [hrp1, hrep]) hfr1
            (by rw [hrd1]; exact hrad) (bnd_congr hrr1 hrp1 hbd)
...
/-- **`replay_after_fallback_general''_stage`.**  `replay_after_fallback_general''`
...
theorem replay_after_fallback_general''_stage (raw : List (Fin 2)) (P : Shared)
    (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM) (q : ℕ) (first : Fin 9)
    (delay : ℕ) (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
  ...
  rcases replay_construct3S (q := q) (first := first) hP hP' hex hd hsearch hpres hshape hbudget hrs hB
      r c t 0
      ⟨⟨hm, hc, by rw [hrpl]; simp [hr0], hrep, hR.1, searchReady_of_restarted hR, hM,
...
#print axioms stage_arith
#print axioms replayChainSeg_canonical
#print axioms restartShapeL_sharedC
#print axioms restartShape_of_L
#print axioms chainW_break'
#print axioms chain_breakWS
#print axioms chain_runWS
#print axioms window_of_foundS
#print axioms found_resultS
#print axioms idle_countdown3S
#print axioms idle_compare3S
#print axioms replay_construct3S
#print axioms replay_after_fallback_general''_stage

/-! ## 12. The chain-start shape at the found ticks of a replay

`GalilWatchOkInst.StartShape` is false for every `P` (`GalilLeafStartShape`):
a search already in `.found` stutters with an arbitrary `vq.dp`.  The shape is
only true of the found ticks a replay reaches, i.e. under `ReplayStageD` (and,
for a `true` arrival, at the comparison tick `clock = 1`) — exactly where the
construction uses it.  `startShape'_of_decodes` proves it for the decoding
machine from `GalilLaterRadius.first_found` (background tick) and
`found_radius_le_all_stages` (comparison tick): the DP output tape at `pc = 346`
is `output h` with head `h`, hence `AnswerExact … h`, and the candidate `h` has
`4h+1` places in the window, hence `PlaceAhead … h`. -/
...
/-- The chain-start shape at the found ticks of a replay run. -/
def StartShape' (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ) : Prop :=
  ∀ (c : Control) (s : GalilVM) (a : Bool) (vq : SearchVM),
    c.mode = .scan → c.replaying = true → s.chain = .idle → SearchReady (searchLens.get s) →
    searchEffect P a s vq → vq.search.mode = .found → (a = false ∨ c.clock = 1) →
    ReplayStageD raw P q first delay c s →
    ∃ n, 0 < n ∧ AnswerAhead (vq.dp.config.tapes 11) n ∧ PlaceAhead (P.place s) n

theorem read_eq_getD (L : List (Fin 9)) : ∀ i, GalilScaffoldTape.read L i = (L[i]?).getD 6 := by
  induction L with
  ...
  | cons a L ih =>
    ...
    cases i with
    | zero => rfl
    | succ i => simp [GalilScaffoldTape.read, ih]

/-- The DP output tape `output h` with head `h` is the exact answer `h`. -/
theorem answerExact_of_output {t : GalilScaffoldTape.Tape} {h : ℕ}
    (hden : GalilScaffoldTape.denote t = GalilDpCounters.output h)
    (hpos : GalilScaffoldTape.head t = h) : GalilShiftH.AnswerExact t h := by
  unfold GalilShiftH.AnswerExact
  have hlen : t.left.length = h := hpos
  have hcell : ∀ i, (t.left.reverse ++ t.focus :: t.right)[i]?.getD 6 = GalilDpCounters.output h i := by
    intro i
    have := congrFun hden i
    rw [GalilScaffoldTape.denote, read_eq_getD] at this
    exact this
  have hfocus : t.focus = GalilDpCounters.output h h := by
    have := hcell h
    rw [List.getElem?_append_right (by simp; omega), List.length_reverse, hlen, Nat.sub_self] at this
    simpa using this
  cases h with
  | zero =>
    have hl : t.left = [] := List.eq_nil_of_length_eq_zero hlen
    rw [hl, hfocus]; rfl
  | succ m =>
    have hf : t.focus = 8 := by rw [hfocus]; simp [GalilDpCounters.output]
    have hl : t.left = List.replicate m 8 ++ [4] := by
      apply List.ext_getElem?
      intro j
      by_cases hj : j < m + 1
      · have hrev : t.left[j]? = t.left.reverse[m - j]? := by
          rw [List.getElem?_reverse (by rw [hlen]; omega), hlen]
          congr 1; omega
        have hc := hcell (m - j)
        rw [List.getElem?_append_left (by simp; omega)] at hc
        rw [List.getElem?_eq_getElem (by simp; omega)] at hc
        rw [hrev, List.getElem?_eq_getElem (by simp; omega), List.getElem?_eq_getElem (by simp; omega)]
...
/-- The found search at a background tick of the replay: the DP output. -/
theorem found_bg_data (P : Shared) (qq : ℕ) (first : Fin 9) (hP : Decodes P)
  ...
  exact ⟨k, h, span, hres, hpc, hpos, hcand, h1⟩

/-- **`StartShape'` for the decoding machine** (`delay = 2048`). -/
theorem startShape'_of_decodes {raw : List (Fin 2)} {P : Shared} {q : ℕ} {first : Fin 9}
    (hP : Decodes P) : StartShape' raw P q first 2048 := by
  intro c s a vq hm hr hidle hsr hq hf hac hst
  obtain ⟨r, Rad, last, es, c0, hR, hSt, hcl, hseg⟩ := hst
  obtain ⟨al, ls, rs, qq', hdec, -⟩ := represents_decompose r.center raw hR.2.1 hR.2.2.1
  have hcen1 : s.center = r.center := watchSegE_center P q first 2048 hseg
  obtain ⟨-, hplr⟩ := hP.1 r al ls rs qq' r.center.gap hdec
  have hplace : P.place s = ⟨al :: ls, r.center.gap⟩ := by rw [hP.2 s r hcen1, hplr]
  obtain ⟨k, h, span, hres, hpc, hpos, hcand, h1⟩ : ∃ k h span : ℕ,
      GalilDpCorrect.Result ((GalilScaffoldPlace.stream ⟨al :: ls, r.center.gap⟩).take (span+1)) k 0
        (GalilScaffoldProgram.denote vq.dp.config) ∧
      (GalilScaffoldProgram.denote vq.dp.config).pc = 346 ∧
      (GalilScaffoldProgram.denote vq.dp.config).pos 11 = h ∧
      GalilDpCorrect.Candidate ((GalilScaffoldPlace.stream ⟨al :: ls, r.center.gap⟩).take (span+1)) k h ∧
      1 ≤ h := by
    cases a with
    | false =>
      exact found_bg_data P q first hP al ls rs qq' r.center.gap hR hdec hcl hSt hseg hidle vq hq hf
    | true =>
      have hc1 : c.clock = 1 := by
        rcases hac with h0 | h0
        · cases h0
        · exact h0
      obtain ⟨k, h, span, -, hres, hpc, hpos, hcand, -, -, h1, -⟩ :=
        GalilLaterRadius.found_radius_le_all_stages P q first hP al ls rs qq' r.center.gap hR hdec hcl
          hSt hseg hidle hc1 vq hq hf
      exact ⟨k, h, span, hres, hpc, hpos, hcand, h1⟩
  refine ⟨h, by omega, ?_, ?_⟩
  · rcases hres with ⟨k2, -, -, -, -, htape, hpos2⟩ | ⟨hpc', -⟩
    · have hk2 : k2 = h := by rw [← hpos, ← hpos2]
      subst hk2
      exact GalilShiftH.answerExact_ahead (answerExact_of_output htape hpos2)
...
/-- `replay_after_fallback_general''_stage'` at `delay = 2048` for a decoding
machine: no chain-start hypothesis at all. -/
theorem replay_after_fallback_general''_stage'_of_decodes (raw : List (Fin 2)) (P : Shared)
    (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM) (q : ℕ) (first : Fin 9)
    (hex : ∀ s, P.replayExhausted s = zero s.replay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
  ...
  replay_after_fallback_general''_stage' raw P hP hP' q first 2048 hex (by omega) hsearch hpres
    (startShape'_of_decodes hdec) hbudget hrs r hr0 c t hm hc hrpl hR hrep hM hfr hsi

end StageConstruct'

#print axioms answerExact_of_output
#print axioms found_bg_data
#print axioms startShape'_of_decodes
#print axioms replay_construct3S'
#print axioms replay_after_fallback_general''_stage'
#print axioms replay_after_fallback_general''_stage'_of_decodes

/-! ## 13. The search readiness from the stage, without `hpres`

`hpres` (every search effect preserves `SearchReady`) is false
(`GalilLeafPres`).  What is true is the budgeted readiness `SearchReadyB` of
`GalilLeafPres` (`ReadyRem` plus the `.run`-entry data `RunEntriesAll`), which one
`searchStep` consumes one event of.  Those modules import this one, so the
notions are restated here verbatim (`RunEntriesAllD`, `SearchReadyBD`,
`ReadyFuelD`; `RunEntriesAllD as v ↔ RunEntriesAll as v` by induction on `as`).

The construction does not even need fuel accounting: every idle state of a
replay carries `ReplayStageD`, i.e. it ends a chain-idle segment out of a
`Restarted` state, whose search is `begin last radius`.  Under the one named
hypothesis `RunEntriesAtBegin` (the `.run`-entry data at a restarted search —
the `hE` of `GalilSegmentConstructB.readyFuel_restarted`, for every restart)
the restarted search is `SearchReadyBD` for *every* event list, and that is
preserved along the segment (`searchReadyBD_watchSegE`), so
`SearchReady (searchLens.get s)` holds at every stage state
(`searchReady_of_stage`) and `ReadyFuelD (searchLens.get s) n K` for every
`n K` (`readyFuel_of_stage`). -/
...
/-- `GalilLeafPres.RunEntriesAll`, restated. -/
def RunEntriesAllD : List Bool → SearchVM → Prop
  | [], _ => True
  | a :: as, v =>
      ∀ (center : GalilScaffoldPlace.Place) (v' : SearchVM), searchStep center a v v' →
        GalilSearchReadyInv.RunEntry center a v v' as ∧ RunEntriesAllD as v'

/-- `GalilLeafPres.SearchReadyB`, restated. -/
def SearchReadyBD (v : SearchVM) (as : List Bool) : Prop :=
  GalilSearchReadyInv.ReadyRem v as ∧ RunEntriesAllD as v

/-- `GalilSegmentConstructB.ReadyFuel`, restated. -/
def ReadyFuelD (v : SearchVM) (n K : ℕ) : Prop :=
  ∀ as : List Bool, n ≤ as.length → as.count true ≤ K → SearchReadyBD v as

/-- **Named hypothesis.**  The `.run`-entry data of a restarted search, for every
event list (`readyFuel_restarted`'s `hE`, at every restart). -/
...
theorem searchReadyBD_ready {v : SearchVM} {as : List Bool} (h : SearchReadyBD v as) :
    SearchReady v :=
  GalilSearchReadyInv.searchReady_of_readyRem h.1

theorem searchReadyBD_step {center : GalilScaffoldPlace.Place} {a : Bool} {as : List Bool}
    {v v' : SearchVM} (hstep : searchStep center a v v') (h : SearchReadyBD v (a :: as)) :
    SearchReadyBD v' as := by
  obtain ⟨hrem, hE⟩ := h
  obtain ⟨hentry, hrest⟩ := hE center v' hstep
  exact ⟨GalilSearchReadyInv.searchReady_step center a as hstep hrem hentry, hrest⟩

/-- A search effect (chain idle or not) keeps the all-lists readiness. -/
theorem searchReadyBD_effect_all (P : Shared) {a : Bool} {s : GalilVM} {v : SearchVM}
    (he : searchEffect P a s v) (hall : ∀ as, SearchReadyBD (searchLens.get s) as) :
    ∀ as, SearchReadyBD v as := by
  intro as
  rcases he with ⟨_, hs⟩ | ⟨_, rfl⟩
  · exact searchReadyBD_step hs (hall (a :: as))
  · exact hall as

variable {raw : List (Fin 2)} {P : Shared} {q : ℕ} {first : Fin 9} {delay : ℕ}

/-- The all-lists readiness travels along a chain-idle segment. -/
theorem searchReadyBD_watchSegE {es : List Bool} {c c' : Control} {s t : GalilVM}
    (h : WatchSegE P q first delay es c s c' t)
    (hall : ∀ as, SearchReadyBD (searchLens.get s) as) :
    ∀ as, SearchReadyBD (searchLens.get t) as := by
  induction h with
  | stop c s => exact hall
  | wait c s s' hm hr hn hb _ ih =>
    exact ih (searchReadyBD_effect_all P
      (backgroundS_fields P q first hb).2.2.2.2.2.2.2.2.2.2.2 hall)
  ...
  | «match» c s vs vq o hm hr ha hc hne hcmp hmt hq ho _ ih =>
    exact ih (searchReadyBD_effect_all P hq hall)
  ...
  | matchIdleR c s vs vq o hm hr hc ha hidle hl hrr hvs hmt hq hnf ho _ ih =>
    apply ih
    rw [replayDec_search]
    exact searchReadyBD_effect_all P hq hall

/-- **Readiness at every stage state.** -/
theorem stage_readyB (hE : RunEntriesAtBegin) {c : Control} {s : GalilVM}
    (hst : ReplayStageD raw P q first delay c s) :
    ∀ as, SearchReadyBD (searchLens.get s) as := by
  obtain ⟨r, Rad, last, es, c0, hR, -, -, hseg⟩ := hst
  refine searchReadyBD_watchSegE hseg (fun as => ⟨GalilSearchReadyInv.searchReady_restarted hR as, ?_⟩)
  exact hE (searchLens.get r) last r.radius hR.2.2.2.2.2.2.1 as

theorem searchReady_of_stage (hE : RunEntriesAtBegin) {c : Control} {s : GalilVM}
    (hst : ReplayStageD raw P q first delay c s) : SearchReady (searchLens.get s) :=
  searchReadyBD_ready (stage_readyB hE hst [])

theorem readyFuel_of_stage (hE : RunEntriesAtBegin) {c : Control} {s : GalilVM}
    (hst : ReplayStageD raw P q first delay c s) (n K : ℕ) :
    ReadyFuelD (searchLens.get s) n K :=
  fun as _ _ => stage_readyB hE hst as

end Fuel

section StageConstructF
...
/-- The countdown of an idle replay stretch, carrying the stage. -/
theorem idle_countdown3SF (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesAtBegin)
    (hshape : StartShape' raw P q first delay) (hbudget : ReplayBudgetRD raw P q first delay)
    (hrs : RestartShapeL P) (hBl : B < (encoded raw).length) {r : ℕ}
    (ih : ∀ m' < r, ∀ (c : Control) (s : GalilVM) (k : ℕ), EntryS raw P q first delay B c s k m' →
      Result3S raw P q first delay B c s k m') :
  ...
  | succ j ihj =>
    ...
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
        idle_background_exists P q first s hidle hv hf
      have hsr' : SearchReady (searchLens.get s') :=
        searchReady_of_stage hE (replayStageD_trans hst (.countR c s s' hm hr hclt hidle hb (.stop _ _)))
      have hfld := backgroundS_fields P q first hb
      have hrem' : s'.remaining = s.remaining := hfld.2.2.2.2.2.2.2.2.1
      have hrad' : s'.radius = s.radius := hfld.2.2.2.2.2.1
      have hlen' : s'.length = s.length := hfld.2.2.2.2.2.2.1
...
/-- The comparison of an idle replay stretch, carrying the stage; the quiet
branch exports the output soundness of the refreshed state. -/
theorem idle_compare3SF (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesAtBegin)
  ...
  · -- a quiet comparison
    ...
    · have ht := scan_match_idle_S' P q first delay c s vs vq o hm (Or.inl hr) hc hidle rfl rfl rfl
      ...
      exact .succ (GalilReplaySegment.soundScanNR_replaying raw hr) (by simpa using ht)
        (.zero _ (fun _ _ => hout))
    · rw [hudef, replayDec_chain, afterCompare_chain]
    · exact searchReady_of_stage hE (replayStageD_trans hst
        (.matchIdleR c s vs vq o hm hr hc hav hidle rfl rfl rfl hmt hq hf ho (.stop _ _)))
    · exact minv_matchR P hex o delay hr rfl hav hi hM
    · rw [hur]; exact hpos
    · rw [hudef, replayDec_center, afterCompare_center]
...
/-- **The idle replay with chain starts, breaks and restarts, carrying the
stage**, by strong induction on the comparisons left. -/
theorem replay_construct3SF (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesAtBegin)
  ...
  | _ r ih =>
    ...
    | succ n =>
      have hrt : c.replaying = true := by rw [hrp]; simp
      rcases idle_countdown3SF (q := q) (first := first) hP hP' hex hd hsearch hE hshape hbudget hrs
          hBl ih (delay - 1) c s k n le_rfl hm hrt (by omega) hidle hsr hrep hM hi hfr hrad hbd
          ⟨hcen1, hcen2⟩ hst hlen with hA | hB1
      · obtain ⟨c1, t1, hseg1, hstp1, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
          hrd1, hst1, hln1⟩ := hA
        ...
        have hi1 : ScanInvariant raw (position t1.center) k t1.left t1.right := by
          rw [hl1, hrr1, hC1]; exact hi
        have hfr1 : Frontier t1 := frontier_congr hrr1 hrp1 hfr
        rcases idle_compare3SF (q := q) (first := first) hP hP' hex hd hsearch hE hshape hbudget hrs
            hBl ih c1 t1 k n le_rfl hm1' hr1' hc1 hidle1 hsr1 hM1 hi1 (by rw [hrp1, hrep]) hfr1
            (by rw [hrd1]; exact hrad) (bnd_congr hrr1 hrp1 hbd)
            (by rw [hC1]; exact ⟨hcen1, hcen2⟩) hst1 (by rw [hln1]; exact hlen) with hA2 | hB2
        · obtain ⟨c2, u, hseg2, hstp2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2,
...
theorem replay_after_fallback_general''_fuel (raw : List (Fin 2)) (P : Shared)
  ...
  have hst0 : ReplayStageD raw P q first delay c t :=
    ⟨t, 0, reset, [], c, hR, stageEntry_zero reset, hc, .stop _ _⟩
  rcases replay_construct3SF (q := q) (first := first) hP hP' hex hd hsearch hE hshape hbudget hrs hB
      r c t 0
      ⟨⟨hm, hc, by rw [hrpl]; simp [hr0], hrep, hR.1, searchReady_of_restarted hR, hM,
        hR.2.2.2.1, hfr, hR.2.2.2.2.1, hbd, hR.2.1, hR.2.2.1⟩, hst0, hR.2.2.2.2.2.1,
        fun h0 => absurd h0 (by omega)⟩ with hQ | hBusy
...
machine: no chain-start hypothesis at all. -/
theorem replay_after_fallback_general''_fuel_of_decodes (raw : List (Fin 2)) (P : Shared)
    (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM) (q : ℕ) (first : Fin 9)
    (hex : ∀ s, P.replayExhausted s = zero s.replay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    ...
    (BrokeAndRestarted raw P q first 2048 c t ∧
      ∃ (n : ℕ) (c' : Control) (t' : GalilVM),
        ...
        position t'.right = position t.right + r ∧ t'.center = t.center ∧
        GalilReplaySegment.InvScan 2048 raw c' t' r ∧ OutputRel raw c' t') :=
  replay_after_fallback_general''_fuel raw P hP hP' q first 2048 hex (by omega) hsearch hE
    (startShape'_of_decodes hdec) hbudget hrs r hr0 c t hm hc hrpl hR hrep hM hfr hsi
...
#print axioms readyFuel_of_stage
#print axioms replay_construct3SF
...
/-! ## 15. The one-stage begin-side readiness, with the replay's comparison count

`RunEntriesPaced` is false (`CloseoutRunEntriesPaced`): a paced list that is
merely too long asks for more comparisons than the stage debt.  The stage debt
at the `.run` entry of the stage a restart `begin last radius` dispatches is
`2·max k 1 - Rad` (`initialDebt radius = -Rad`, `+2` per grow tick, `max k 1`
grow ticks for `value last = k`): `stageDebt`.  `RunEntriesStage delay` is the
paced statement with that side condition; it is the ONE named hypothesis of this
section (the concrete proof is the calibrated-stage lemmas
`runEntries_of_calibrated` / `runEntries_two_stages`, not assembled here).

The construction then carries the comparison count: `ReplayStageDC … c s K`
adds to `ReplayStageD` that the segment's comparisons plus the `K` still to be
made stay within the stage debt.  The count is `0` at a restart, grows by one
per comparison, and the entry inequality — the replay's remaining comparisons
fit the stage debt of the restart that begins it — is the second named fact
`ReplayFitsStage raw` (used at the replay entry, `Rad = 0`, and at every
mid-replay restart). -/
...
/-- The debt at the `.run` entry of the stage begun by `begin last radius`
(`value last = k`, `value radius = Rad`). -/
def stageDebt (Rad : ℕ) (k : ℤ) : ℤ := 2 * max k 1 - Rad

/-- **Named hypothesis.**  The `.run`-entry data of a `StageEntry` restart, for
every clock-paced event list whose comparisons fit the stage debt. -/
def RunEntriesStage (delay : ℕ) : Prop :=
  ∀ (v : SearchVM) (last radius : Counter) (Rad : ℕ),
    v.search = GalilScaffoldSearchFinish.begin last radius → Canonical last → 0 ≤ value last →
    Canonical radius → value radius = Rad → StageEntry Rad last →
    ∀ av : List Bool,
      (((GalilScaffoldAdvanceClock.advances delay delay (av.map (fun b => (b, true)))).count true
        : ℕ) : ℤ) ≤ stageDebt Rad (value last) →
      RunEntriesAllD (GalilScaffoldAdvanceClock.advances delay delay (av.map (fun b => (b, true)))) v

/-- **Named hypothesis.**  The comparisons a restart still has to replay fit the
stage debt of the stage it begins. -/
def ReplayFitsStage (raw : List (Fin 2)) : Prop :=
  ∀ (r : GalilVM) (Rad : ℕ) (last : Counter) (m : ℕ),
    Restarted raw r Rad last → StageEntry Rad last → r.replay = ofNat m →
    (m : ℤ) ≤ stageDebt Rad (value last)

/-- `ReplayStageD` with the comparison count: the segment's comparisons plus the
`K` still to come fit the stage debt. -/
def ReplayStageDC (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ)
    (c : Control) (s : GalilVM) (K : ℕ) : Prop :=
  ∃ (r : GalilVM) (Rad : ℕ) (last : Counter) (es : List Bool) (c0 : Control),
    Restarted raw r Rad last ∧ StageEntry Rad last ∧ c0.clock = delay ∧
      WatchSegE P q first delay es c0 r c s ∧
      ((es.count true + K : ℕ) : ℤ) ≤ stageDebt Rad (value last)

/-- `EntryS` with the counted stage datum. -/
def EntryC (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay B : ℕ)
    (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  Entry raw delay B c s k r ∧ ReplayStageDC raw P q first delay c s r ∧ Canonical s.length ∧
    (r = 0 → OutputRel raw c s)

/-- `LiveEndS` with the counted stage datum in the break branch. -/
def LiveEndC (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay B : ℕ)
    (cc b : Fin 3) (xs : List (Fin 3)) (c : Control) (s : GalilVM) (k m : ℕ) : Prop :=
  (∃ (es : List Bool) (c' : Control) (t : GalilVM),
      ReplayChainSeg P q first delay es c s c' t ∧ es.count true = m ∧
      c'.mode = .scan ∧ c'.clock = delay ∧ c'.replaying = false ∧ t.replay = reset ∧
  ...
  (∃ (es : List Bool) (c' : Control) (t : GalilVM) (m' : ℕ),
      ...
      t.center = s.center ∧ Frontier t ∧ t.remaining = s.remaining ∧ Bnd B t ∧
      (P.onLetter = onLetterVM raw → P.leftFirst = leftFirstVM → m' = 0 → OutputRel raw c' t) ∧
      ReplayStageDC raw P q first delay c' t m' ∧ Canonical t.length)

variable {raw : List (Fin 2)} {P : Shared} {q : ℕ} {first : Fin 9} {delay : ℕ}

theorem replayStageD_of_DC {c : Control} {s : GalilVM} {K : ℕ}
    (h : ReplayStageDC raw P q first delay c s K) : ReplayStageD raw P q first delay c s := by
  obtain ⟨r, Rad, last, es, c0, hR, hSt, hcl, hseg, -⟩ := h
  exact ⟨r, Rad, last, es, c0, hR, hSt, hcl, hseg⟩

theorem replayStageDC_trans {c c' : Control} {s s' : GalilVM} {es : List Bool} {K K' : ℕ}
    (hst : ReplayStageDC raw P q first delay c s K) (hseg : WatchSegE P q first delay es c s c' s')
    (hK : es.count true + K' ≤ K) : ReplayStageDC raw P q first delay c' s' K' := by
  obtain ⟨r, Rad, last, es0, c0, hR, hSt, hcl, hseg0, hcnt⟩ := hst
  refine ⟨r, Rad, last, es0 ++ es, c0, hR, hSt, hcl, watchSegE_trans P q first delay hseg0 hseg, ?_⟩
  rw [List.count_append]
  have : ((es0.count true + es.count true + K' : ℕ) : ℤ) ≤ ((es0.count true + K : ℕ) : ℤ) := by
    exact_mod_cast (by omega : es0.count true + es.count true + K' ≤ es0.count true + K)
  exact le_trans this hcnt

/-- **The next paced event is covered at every counted stage state.** -/
theorem stage_readyB'' (hE : RunEntriesStage delay) (hdec : Decodes P) {c : Control} {s : GalilVM}
    {K : ℕ} (hst : ReplayStageDC raw P q first delay c s K) (hidle : s.chain = .idle) (a : Bool)
    (ha : a = true → c.clock = 1) (hK : a = true → 1 ≤ K) :
    SearchReadyBD (searchLens.get s) [a] := by
  obtain ⟨r, Rad, last, es, c0, hR, hSt, hcl0, hseg, hcnt⟩ := hst
  have hrun := watchSegE_searchRun P q first delay hdec.2 hseg hidle
  obtain ⟨av, -, hes, hcl, -⟩ := watchSegE_clock P q first delay hseg
  have hev : es ++ [a] = GalilScaffoldAdvanceClock.advances delay c0.clock
      ((av ++ [a]).map (fun b => (b, true))) := by
    ...
    | false => simp [GalilScaffoldAdvanceClock.advances]
    | true => simp [GalilScaffoldAdvanceClock.advances, ha rfl]
  have hcnt' : (((es ++ [a]).count true : ℕ) : ℤ) ≤ stageDebt Rad (value last) := by
    refine le_trans ?_ hcnt
    rw [List.count_append]
    have : (es ++ [a]).count true ≤ es.count true + K := by
      rw [List.count_append]
      cases a with
      | false => simp
      | true => simp; exact hK rfl
    exact_mod_cast (by rw [List.count_append] at this; exact this)
  have hall : RunEntriesAllD (es ++ [a]) (searchLens.get r) := by
    rw [hev, hcl0]
    rw [hev, hcl0] at hcnt'
    exact hE (searchLens.get r) last r.radius Rad hR.2.2.2.2.2.2.1 hR.2.2.2.2.2.2.2.2.1
      hR.2.2.2.2.2.2.2.2.2 hR.2.2.2.2.1.1 hR.2.2.2.2.1.2 hSt (av ++ [a]) hcnt'
  exact searchReadyBD_searchRun hrun [a]
    ⟨GalilSearchReadyInv.searchReady_restarted hR (es ++ [a]), hall⟩

theorem searchReady_of_stage'' (hE : RunEntriesStage delay) (hdec : Decodes P) {c : Control}
    {s : GalilVM} {K : ℕ} (hst : ReplayStageDC raw P q first delay c s K) (hidle : s.chain = .idle)
    {a : Bool} (ha : a = true → c.clock = 1) (hK : a = true → 1 ≤ K) {v : SearchVM}
    (he : searchEffect P a s v) : SearchReady v :=
  searchReadyBD_ready (searchReadyBD_effect P hidle (stage_readyB'' hE hdec hst hidle a ha hK) he)
...
stage.**  `chain_breakW` with the restarted state shown to be
`Restarted raw u (k+1) last` with `StageEntry (k+1) last`. -/
theorem chain_breakWC (hex : ∀ s, P.replayExhausted s = zero s.replay) (hrs : RestartShapeL P) (hfit : ReplayFitsStage raw)
    (hBl : B < (encoded raw).length)
    (c : Control) (s : GalilVM) (k m : ℕ)
    (hm : c.mode = .scan) (hr : c.replaying = true) (hc : c.clock = 1)
    (hpart : ChainW raw (position s.center) E (position s.right) (budOf delay E c s) true cc b xs s.chain)
    ...
    ∃ (c1 : Control) (u : GalilVM),
      ...
      RadiusRep u.radius (k+1) ∧ Bnd B u ∧
      (P.onLetter = onLetterVM raw → P.leftFirst = leftFirstVM → OutputRel raw c1 u) ∧
      ReplayStageDC raw P q first delay c1 u m ∧ Canonical u.length := by
  have hne : s.chain ≠ .idle := chainW_ne_idle hpart
  have hbud : budOf delay E c s = 0 + 1 := by
    unfold budOf; rw [hc, hRE]; simp
  rw [hbud] at hpart
  ...
  refine ⟨{cmpC P delay c s (.broken w') with clock := delay}, s',
    ...
    frontier_congr hr' hrep' hfru, by rw [hrem', hremu], hrad'',
    bnd_congr hr' hrep' hbdu, ?_, ⟨s', k+1, w'.machine.control.last, [], _, hRst, hSt, rfl, .stop _ _,
      by simpa using hfit s' (k+1) w'.machine.control.last m hRst hSt (by rw [hrep', hurep])⟩,
    hclen⟩
  · apply searchReady_of_begin (last := w'.machine.control.last)
    rw [hsearch', hrad']
  · exact minv_same rfl hr' hC' hrep' hMu
...
/-- **The live chain along the rest of the replay**, carrying the stage into
the break branch. -/
theorem chain_runWC (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hrs : RestartShapeL P) (hfit : ReplayFitsStage raw) (hBl : B < (encoded raw).length) (hEB : E ≤ B)
    (hlimF : lim = false → E = B) (hlimT : lim = true → E < B) :
    ∀ (m : ℕ) (c : Control) (s : GalilVM) (k : ℕ),
      c.mode = .scan → 1 ≤ c.clock → c.replaying = decide (0 < m) → (m = 0 → c.clock = delay) →
      s.replay = ofNat m → Bnd B s →
      ...
      (GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) →
      Canonical s.length →
      LiveEndC raw P q first delay B cc b xs c s k m := by
  intro m
  induction m with
  | zero =>
    intro c s k hm hclk hrp hc0 hrep hbd hpart hRE hbrk hM hi hfr hrad _ _
  ...
  | succ n ih =>
    ...
    · have hRE1 : position s1.right = E := by rw [hr1] at hlt ⊢; omega
      ...
      | true =>
        ...
        obtain ⟨c2, u, hseg2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2, hrem2, hrad2,
            hbd2, hout2, hst2, hcan2⟩ :=
          chain_breakWC raw P q first delay B E cc b xs hex hrs hfit hBl {c with clock := 1} s1 k n hm hrt rfl
            hp1 hRE1 (hlimT rfl) (by rw [hC1]; exact hmis) (by rw [hC1]; exact h4) hbd1 hM1 hi1 hrep1'
            hfr1 hrad1' hcen1 hcan1
        refine Or.inr ⟨es1 ++ [true], c2, u, n,
          replayChainSeg2_trans (replayChainSeg2_of_seg hseg1) hseg2, by omega, hm2, hc2, hr2, hrep2,
...
/-- **From a found tick to a busy landing**, with the stage re-entered after a
break. -/
theorem found_resultC (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    {c c2 : Control} {s s2 : GalilVM} {k r r0 k2 m2 : ℕ} {cc b : Fin 3} {xs : List (Fin 3)}
    (ih : ∀ m' < r0, ∀ (c : Control) (s : GalilVM) (k : ℕ), EntryC raw P q first delay B c s k m' →
      Result3S raw P q first delay B c s k m')
    (hidle : s.chain = .idle) (hrt : c.replaying = true)
    (hft : FoundTick P q first delay c s c2 s2)
    (hst1 : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩ ⟨c2, s2⟩)
    (hQ2 : SoundScanNR raw ⟨c2, s2⟩) (hne2 : s2.chain ≠ .idle)
    (hi2 : ScanInvariant raw (position s2.center) k2 s2.left s2.right)
    (hlive : LiveEndC raw P q first delay B cc b xs c2 s2 k2 m2)
    (hk : k2 + m2 = k + r) (hp : position s2.right + m2 = position s.right + r)
    (hC : s2.center = s.center) (hrem : s2.remaining = s.remaining) (hmr : m2 ≤ r) (hrr0 : r ≤ r0)
    (hcen : GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) :
    BusyS raw P q first delay B c s k r := by
...
/-- The countdown of an idle replay stretch, carrying the stage. -/
theorem idle_countdown3SC (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesStage delay) (hdec : Decodes P) (hfit : ReplayFitsStage raw)
    (hshape : StartShape' raw P q first delay) (hbudget : ReplayBudgetRD raw P q first delay)
    (hrs : RestartShapeL P) (hBl : B < (encoded raw).length) {r : ℕ}
    (ih : ∀ m' < r, ∀ (c : Control) (s : GalilVM) (k : ℕ), EntryC raw P q first delay B c s k m' →
      Result3S raw P q first delay B c s k m') :
    ∀ (j : ℕ) (c : Control) (s : GalilVM) (k m : ℕ), m + 1 ≤ r → c.mode = .scan →
      c.replaying = true →
      c.clock = j + 1 → s.chain = ChainVM.idle → SearchReady (searchLens.get s) →
      ...
      RadiusRep s.radius k → Bnd B s →
      (GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) →
      ReplayStageDC raw P q first delay c s (m+1) → Canonical s.length →
      (∃ (c1 : Control) (t : GalilVM),
        WatchSegE P q first delay (List.replicate j false) c s c1 t ∧
        StepsAll (galilFrameS P q first) delay (SoundScanNR raw) j ⟨c, s⟩ ⟨c1, t⟩ ∧
        c1.mode = c.mode ∧ c1.replaying = c.replaying ∧ c1.clock = 1 ∧
        ...
        t.left = s.left ∧ t.right = s.right ∧ t.center = s.center ∧
        t.replay = s.replay ∧ t.remaining = s.remaining ∧ t.radius = s.radius ∧
        ReplayStageDC raw P q first delay c1 t (m+1) ∧ t.length = s.length) ∨
      BusyS raw P q first delay B c s k (m+1) := by
  intro j
  induction j with
  | zero =>
  ...
  | succ j ihj =>
    ...
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
      ...
      have hrad' : s'.radius = s.radius := hfld.2.2.2.2.2.1
      have hlen' : s'.length = s.length := hfld.2.2.2.2.2.2.1
      obtain ⟨n, hn, ha, hpl⟩ := hshape c s false v hm hr hidle hsr hv hf (Or.inl rfl) (replayStageD_of_DC hst)
      obtain ⟨xs, b, hsplit⟩ := stream_split hpl hn
      obtain ⟨E, lim, hE1, hlimF, hlimT, hRE, hwin, hbrk⟩ :=
        window_of_foundS hbudget c s false v k (m+1) n xs b hm hr hidle hsr hv hf hM hi hrad hrep
          (Nat.succ_pos m) hfr ha hn hpl hsplit (replayStageD_of_DC hst)
      have hBeq : position s.right + (m+1) = B := hbd (m+1) hrep
      have hpart0 : ChainW raw (position s'.center) E (position s'.right)
          (budOf delay E {c with clock := c.clock - 1} s') lim (P.centre s) b xs s'.chain := by
        rw [hch', hC', hr']
        ...
        have h1 := (hbrk hl).2.2
        unfold budOf; rw [hr']; simp only; omega
      have hlive := chain_runWC raw P q first delay B E lim (P.centre s) b xs hex hd hrs hfit hBl (by omega)
        (fun h => by rw [← hBeq]; exact hlimF h) (fun h => by rw [← hBeq]; exact hlimT h)
        (m+1) {c with clock := c.clock - 1} s' k hm (by simp; omega) (by simpa using hr)
        (fun h => absurd h (Nat.succ_ne_zero m)) (by rw [hrep', hrep]) (bnd_congr hr' hrep' hbd) hpart0
        (by rw [hr']; omega) (by rw [hC']; exact fun h => ⟨(hbrk h).1, (hbrk h).2.1⟩)
      ...
      have hQ : SoundScanNR raw ⟨{c with clock := c.clock - 1}, s'⟩ :=
        GalilReplaySegment.soundScanNR_replaying raw hr
      exact Or.inr <| found_resultC hP hP' ih hidle hr
        (.bg c s s' hm hr hclt hidle hb (by rw [hget']; exact hf))
        (.succ (GalilReplaySegment.soundScanNR_replaying raw hr)
          (.scan_count c s s' hm (Or.inl hr) hclt hb) (.zero _ hQ))
        hQ (by rw [hch']; exact chainStart_ne_idle _ _ _ _ _) (by rw [hl', hr', hC']; exact hi) hlive
    ...
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
        idle_background_exists P q first s hidle hv hf
      have hsr' : SearchReady (searchLens.get s') := by
        rw [hget']; exact searchReady_of_stage'' hE hdec hst hidle (fun h0 => by cases h0) (fun h0 => by cases h0) hv
      have hfld := backgroundS_fields P q first hb
      have hrem' : s'.remaining = s.remaining := hfld.2.2.2.2.2.2.2.2.1
      have hrad' : s'.radius = s.radius := hfld.2.2.2.2.2.1
      have hlen' : s'.length = s.length := hfld.2.2.2.2.2.2.1
      have hQ' : SoundScanNR raw ⟨{c with clock := c.clock - 1}, s'⟩ :=
        GalilReplaySegment.soundScanNR_replaying raw hr
      have hst' : ReplayStageDC raw P q first delay {c with clock := c.clock - 1} s' (m+1) :=
        replayStageDC_trans hst (.countR c s s' hm hr hclt hidle hb (.stop _ _)) (by simp)
      have hstp1 : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩
          ⟨{c with clock := c.clock - 1}, s'⟩ :=
        .succ (GalilReplaySegment.soundScanNR_replaying raw hr)
          (.scan_count c s s' hm (Or.inl hr) hclt hb) (.zero _ hQ')
...
/-- The comparison of an idle replay stretch, carrying the stage; the quiet
branch exports the output soundness of the refreshed state. -/
theorem idle_compare3SC (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesStage delay) (hdec : Decodes P) (hfit : ReplayFitsStage raw)
    ...
    (hbd : Bnd B s)
    (hcen : GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none)
    (hst : ReplayStageDC raw P q first delay c s (m+1)) (hlen : Canonical s.length) :
    (∃ (c1 : Control) (u : GalilVM),
      WatchSegE P q first delay [true] c s c1 u ∧
      StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩ ⟨c1, u⟩ ∧
      c1.mode = .scan ∧ c1.clock = delay ∧ c1.replaying = decide (0 < m) ∧
      ...
      u.center = s.center ∧ Frontier u ∧ u.remaining = s.remaining ∧
      RadiusRep u.radius (k+1) ∧ Bnd B u ∧
      ReplayStageDC raw P q first delay c1 u m ∧ Canonical u.length ∧ OutputRel raw c1 u) ∨
    BusyS raw P q first delay B c s k (m+1) := by
  classical
  have hbound : position s.right + (m+1) ≤ 2 * arrived s.right := hfr (m+1) hrp
  have hav : canRight s.right := GalilReplaySegment.canRight_of_frontier (Nat.succ_pos m) hbound
  ...
  by_cases hf : vq.search.mode = .found
  · -- the comparison starts the chain
    obtain ⟨n, hn, ha, hpl⟩ := hshape c s true vq hm hr hidle hsr hq hf (Or.inr hc) (replayStageD_of_DC hst)
    obtain ⟨xs, b, hsplit⟩ := stream_split hpl hn
    obtain ⟨E, lim, hE1, hlimF, hlimT, hRE, hwin, hbrk⟩ :=
      window_of_foundS hbudget c s true vq k (m+1) n xs b hm hr hidle hsr hq hf hM hi hrad hrp
        (Nat.succ_pos m) hfr ha hn hpl hsplit (replayStageD_of_DC hst)
  ...
  · -- a quiet comparison
    ...
    · rw [hudef, replayDec_chain, afterCompare_chain]
    · have hgetU : searchLens.get u = vq := by rw [hudef, replayDec_search]; rfl
      rw [hgetU]; exact searchReady_of_stage'' hE hdec hst hidle (fun _ => hc) (fun _ => Nat.succ_pos m) hq
    · exact minv_matchR P hex o delay hr rfl hav hi hM
    · rw [hur]; exact hpos
    · rw [hudef, replayDec_center, afterCompare_center]
    · intro m' hm'
    ...
    · intro m' hm'
      ...
      subst hmm
      rw [hur, hpos]; omega
    · exact replayStageDC_trans hst
        (.matchIdleR c s vs vq o hm hr hc hav hidle rfl rfl rfl hmt hq hf ho (.stop _ _)) (by simp; omega)
    · rw [hudef]
      show Canonical (inc (inc s.length))
      exact inc_canonical _ (inc_canonical _ hlen)
...
/-- **The idle replay with chain starts, breaks and restarts, carrying the
stage**, by strong induction on the comparisons left. -/
theorem replay_construct3SC (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesStage delay) (hdec : Decodes P) (hfit : ReplayFitsStage raw)
    (hshape : StartShape' raw P q first delay) (hbudget : ReplayBudgetRD raw P q first delay)
    (hrs : RestartShapeL P) (hBl : B < (encoded raw).length) :
    ∀ (r : ℕ) (c : Control) (s : GalilVM) (k : ℕ), EntryC raw P q first delay B c s k r →
      Result3S raw P q first delay B c s k r := by
  intro r
  induction r using Nat.strong_induction_on with
  | _ r ih =>
    ...
    | succ n =>
      have hrt : c.replaying = true := by rw [hrp]; simp
      rcases idle_countdown3SC (q := q) (first := first) hP hP' hex hd hsearch hE hdec hfit hshape hbudget hrs
          hBl ih (delay - 1) c s k n le_rfl hm hrt (by omega) hidle hsr hrep hM hi hfr hrad hbd
          ⟨hcen1, hcen2⟩ hst hlen with hA | hB1
      · obtain ⟨c1, t1, hseg1, hstp1, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
          hrd1, hst1, hln1⟩ := hA
        ...
        have hi1 : ScanInvariant raw (position t1.center) k t1.left t1.right := by
          rw [hl1, hrr1, hC1]; exact hi
        have hfr1 : Frontier t1 := frontier_congr hrr1 hrp1 hfr
        rcases idle_compare3SC (q := q) (first := first) hP hP' hex hd hsearch hE hdec hfit hshape hbudget hrs
            hBl ih c1 t1 k n le_rfl hm1' hr1' hc1 hidle1 hsr1 hM1 hi1 (by rw [hrp1, hrep]) hfr1
            (by rw [hrd1]; exact hrad) (bnd_congr hrr1 hrp1 hbd)
            (by rw [hC1]; exact ⟨hcen1, hcen2⟩) hst1 (by rw [hln1]; exact hlen) with hA2 | hB2
        · obtain ⟨c2, u, hseg2, hstp2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2,
...
theorem replay_after_fallback_general''_fuel'' (raw : List (Fin 2)) (P : Shared)
  ...
  have hbd : Bnd (position t.right + r) t := by
    intro m hm'
    rw [ofNat_inj (hrep.symm.trans hm')]
  have hst0 : ReplayStageDC raw P q first delay c t r :=
    ⟨t, 0, reset, [], c, hR, stageEntry_zero reset, hc, .stop _ _,
      by simpa using hfit t 0 reset r hR (stageEntry_zero reset) hrep⟩
  rcases replay_construct3SC (q := q) (first := first) hP hP' hex hd hsearch hE hdec hfit hshape hbudget hrs hB
      r c t 0
      ⟨⟨hm, hc, by rw [hrpl]; simp [hr0], hrep, hR.1, searchReady_of_restarted hR, hM,
        hR.2.2.2.1, hfr, hR.2.2.2.2.1, hbd, hR.2.1, hR.2.2.1⟩, hst0, hR.2.2.2.2.2.1,
        fun h0 => absurd h0 (by omega)⟩ with hQ | hBusy
...
theorem replay_after_fallback_general''_fuel''_of_decodes (raw : List (Fin 2)) (P : Shared)
    ...
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hE : RunEntriesStage 2048) (hfit : ReplayFitsStage raw)
    (hdec : Decodes P) (hbudget : ReplayBudgetRD raw P q first 2048)
    (hrs : RestartShapeL P)
    (r : ℕ) (hr0 : 0 < r) (c : Control) (t : GalilVM)
    (hm : c.mode = Mode.scan) (hc : c.clock = 2048) (hrpl : c.replaying = true)
    ...
    (BrokeAndRestarted raw P q first 2048 c t ∧
      ∃ (n : ℕ) (c' : Control) (t' : GalilVM),
        ...
        position t'.right = position t.right + r ∧ t'.center = t.center ∧
        GalilReplaySegment.InvScan 2048 raw c' t' r ∧ OutputRel raw c' t') :=
  replay_after_fallback_general''_fuel'' raw P hP hP' q first 2048 hex (by omega) hsearch hE hdec hfit
    (startShape'_of_decodes hdec) hbudget hrs r hr0 c t hm hc hrpl hR hrep hM hfr hsi
...
#print axioms stage_readyB''
#print axioms searchReady_of_stage''
#print axioms chain_breakWC
#print axioms replay_construct3SC
#print axioms replay_after_fallback_general''_fuel''
#print axioms replay_after_fallback_general''_fuel''_of_decodes
...
(`ReadyClosure`): `ready` (it implies `SearchReady`), `seg` (it travels along a
chain-idle segment ending with an idle chain), `restart` (it holds at every
`Restarted`/`StageEntry` state with a full clock — the replay entry and every
mid-replay restart, where the residual `RunEntriesS` of
`runEntryS_of_entryPaced` lives).  `replay_after_fallback_general''_R` /
`''_R_of_decodes` need no `hpres`, no `RunEntriesAtBegin/Paced/Stage`, no
`ReplayFitsStage`; instantiating `Rd c s := ∀ n, ∃ k, 2048 ≤ c.clock + k ∧
ReadyPacedS (searchLens.get s) n k` downstream gives `''_S_of_decodes`. -/

section ReadyAbstract
...
/-- The closure a readiness ledger needs for the replay construction. -/
structure ReadyClosure (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ)
    (Rd : Control → GalilVM → Prop) : Prop where
  ready : ∀ (c : Control) (s : GalilVM), Rd c s → SearchReady (searchLens.get s)
  seg : ∀ (es : List Bool) (c c' : Control) (s t : GalilVM),
    WatchSegE P q first delay es c s c' t → t.chain = .idle → Rd c s → Rd c' t
  restart : ∀ (c : Control) (u : GalilVM) (Rad : ℕ) (last : Counter),
    c.mode = .scan → c.clock = delay → Restarted raw u Rad last → StageEntry Rad last → Rd c u

/-- `EntryS` with the ledger. -/
def EntryR (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay B : ℕ)
    (Rd : Control → GalilVM → Prop) (c : Control) (s : GalilVM) (k r : ℕ) : Prop :=
  EntryS raw P q first delay B c s k r ∧ Rd c s

/-- `LiveEndS` with the ledger at the restarted state. -/
def LiveEndR (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay B : ℕ)
    (Rd : Control → GalilVM → Prop)
    (cc b : Fin 3) (xs : List (Fin 3)) (c : Control) (s : GalilVM) (k m : ℕ) : Prop :=
  (∃ (es : List Bool) (c' : Control) (t : GalilVM),
      ReplayChainSeg P q first delay es c s c' t ∧ es.count true = m ∧
      c'.mode = .scan ∧ c'.clock = delay ∧ c'.replaying = false ∧ t.replay = reset ∧
  ...
  (∃ (es : List Bool) (c' : Control) (t : GalilVM) (m' : ℕ),
      ...
      t.center = s.center ∧ Frontier t ∧ t.remaining = s.remaining ∧ Bnd B t ∧
      (P.onLetter = onLetterVM raw → P.leftFirst = leftFirstVM → m' = 0 → OutputRel raw c' t) ∧
      ReplayStageD raw P q first delay c' t ∧ Canonical t.length ∧ Rd c' t)
...
stage.**  `chain_breakW` with the restarted state shown to be
`Restarted raw u (k+1) last` with `StageEntry (k+1) last`. -/
theorem chain_breakWR (hex : ∀ s, P.replayExhausted s = zero s.replay) (hrs : RestartShapeL P) {Rd : Control → GalilVM → Prop}
    (hcl : ReadyClosure raw P q first delay Rd)
    (hBl : B < (encoded raw).length)
    (c : Control) (s : GalilVM) (k m : ℕ)
    (hm : c.mode = .scan) (hr : c.replaying = true) (hc : c.clock = 1)
    (hpart : ChainW raw (position s.center) E (position s.right) (budOf delay E c s) true cc b xs s.chain)
    ...
    ∃ (c1 : Control) (u : GalilVM),
      ...
      RadiusRep u.radius (k+1) ∧ Bnd B u ∧
      (P.onLetter = onLetterVM raw → P.leftFirst = leftFirstVM → OutputRel raw c1 u) ∧
      ReplayStageD raw P q first delay c1 u ∧ Canonical u.length ∧ Rd c1 u := by
  have hne : s.chain ≠ .idle := chainW_ne_idle hpart
  have hbud : budOf delay E c s = 0 + 1 := by
    unfold budOf; rw [hc, hRE]; simp
  rw [hbud] at hpart
  ...
  refine ⟨{cmpC P delay c s (.broken w') with clock := delay}, s',
    ...
    frontier_congr hr' hrep' hfru, by rw [hrem', hremu], hrad'',
    bnd_congr hr' hrep' hbdu, ?_, ⟨s', k+1, w'.machine.control.last, [], _, hRst, hSt, rfl, .stop _ _⟩,
    hclen, hcl.restart _ s' (k+1) w'.machine.control.last hm rfl hRst hSt⟩
  · apply searchReady_of_begin (last := w'.machine.control.last)
    rw [hsearch', hrad']
  · exact minv_same rfl hr' hC' hrep' hMu
  · intro hP hP'
...
/-- **The live chain along the rest of the replay**, carrying the stage into
the break branch. -/
theorem chain_runWR (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hrs : RestartShapeL P) {Rd : Control → GalilVM → Prop}
    (hcl : ReadyClosure raw P q first delay Rd) (hBl : B < (encoded raw).length) (hEB : E ≤ B)
    (hlimF : lim = false → E = B) (hlimT : lim = true → E < B) :
    ∀ (m : ℕ) (c : Control) (s : GalilVM) (k : ℕ),
      c.mode = .scan → 1 ≤ c.clock → c.replaying = decide (0 < m) → (m = 0 → c.clock = delay) →
      s.replay = ofNat m → Bnd B s →
      ...
      (GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) →
      Canonical s.length →
      LiveEndR raw P q first delay B Rd cc b xs c s k m := by
  intro m
  induction m with
  | zero =>
    intro c s k hm hclk hrp hc0 hrep hbd hpart hRE hbrk hM hi hfr hrad _ _
  ...
  | succ n ih =>
    ...
    · obtain ⟨c2, u, hseg2, hm2, hc2, hr2, hp2, hM2, hi2, hrep2, hpos2, hC2, hfr2, hrem2, hrad2, hbd2⟩ :=
      ...
      · obtain ⟨es3, c3, t3, hseg3, hcnt3, hm3, hc3, hr3, hrep3, hp3, hB3, hM3, hi3, hpos3, hC3, hfr3,
        ...
        · rw [hrem3, hrem2, hrem1]
      · obtain ⟨es3, c3, t3, m', hseg3, hlt3, hm3, hc3, hr3, hrep3, hidle3, hsr3, hM3, hi3, hrad3, hpos3,
          hC3, hfr3, hrem3, hbd3, hout3, hst3, hcan3, hrd3⟩ := hB'
        refine Or.inr ⟨es1 ++ ([true] ++ es3), c3, t3, m',
          replayChainSeg2_trans (replayChainSeg2_of_seg hseg1)
            (replayChainSeg2_trans (replayChainSeg2_of_seg hseg2) hseg3),
          by omega, hm3, hc3, hr3, hrep3, hidle3, hsr3, hM3, ?_, ?_, ?_, ?_, hfr3, ?_, hbd3, hout3,
    ...
    · have hRE1 : position s1.right = E := by rw [hr1] at hlt ⊢; omega
      ...
      | true =>
        obtain ⟨hmis, h4⟩ := hbrk rfl
        obtain ⟨c2, u, hseg2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2, hrem2, hrad2,
            hbd2, hout2, hst2, hcan2, hrd2⟩ :=
          chain_breakWR raw P q first delay B E cc b xs hex hrs hcl hBl {c with clock := 1} s1 k n hm hrt rfl
            hp1 hRE1 (hlimT rfl) (by rw [hC1]; exact hmis) (by rw [hC1]; exact h4) hbd1 hM1 hi1 hrep1'
            hfr1 hrad1' hcen1 hcan1
        refine Or.inr ⟨es1 ++ [true], c2, u, n,
          replayChainSeg2_trans (replayChainSeg2_of_seg hseg1) hseg2, by omega, hm2, hc2, hr2, hrep2,
          hidle2, hsr2, hM2, ?_, ?_, ?_, ?_, hfr2, ?_, hbd2, fun hP hP' _ => hout2 hP hP', hst2, hcan2, hrd2⟩
        · rw [show k + (n + 1 - n) = k + 1 from by omega]; exact hi2
        · rw [show k + (n + 1 - n) = k + 1 from by omega]; exact hrad2
        · rw [hpos2, hr1]; omega
        · rw [hC2, hC1]
...
/-- **From a found tick to a busy landing**, with the stage re-entered after a
break. -/
theorem found_resultR (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    {Rd : Control → GalilVM → Prop} {c c2 : Control} {s s2 : GalilVM} {k r r0 k2 m2 : ℕ} {cc b : Fin 3} {xs : List (Fin 3)}
    (ih : ∀ m' < r0, ∀ (c : Control) (s : GalilVM) (k : ℕ), EntryR raw P q first delay B Rd c s k m' →
      Result3S raw P q first delay B c s k m')
    (hidle : s.chain = .idle) (hrt : c.replaying = true)
    (hft : FoundTick P q first delay c s c2 s2)
    (hst1 : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩ ⟨c2, s2⟩)
    (hQ2 : SoundScanNR raw ⟨c2, s2⟩) (hne2 : s2.chain ≠ .idle)
    (hi2 : ScanInvariant raw (position s2.center) k2 s2.left s2.right)
    (hlive : LiveEndR raw P q first delay B Rd cc b xs c2 s2 k2 m2)
    (hk : k2 + m2 = k + r) (hp : position s2.right + m2 = position s.right + r)
    (hC : s2.center = s.center) (hrem : s2.remaining = s.remaining) (hmr : m2 ≤ r) (hrr0 : r ≤ r0)
    (hcen : GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) :
    BusyS raw P q first delay B c s k r := by
  ...
  · obtain ⟨es, c', t, hseg, -, hm', hc', hr', hrep', hpart', hBt, hM', hi', hpos', hC', hfr', hrem'⟩ :=
    ...
    exact Or.inl ⟨0, c, s, c2, s2, es, c', t, 1 + n, cc, b, xs,
      ...
      by rw [hrem', hrem]⟩
  · obtain ⟨es, c', t, m', hseg, hlt, hm', hc', hr', hrep', hidle', hsr', hM', hi', hrad', hpos', hC',
      hfr', hrem', hbd', hout', hstage, hcanl, hrd'⟩ := hB'
    obtain ⟨n, hst⟩ := replayChainSeg2_stepsAll raw hP hP' hseg (position s2.center) k2 hi2 hQ2
    have hres := ih m' (by omega) c' t (k2 + (m2 - m'))
      ⟨⟨⟨hm', hc', hr', hrep', hidle', hsr', hM', hi', hfr', hrad', hbd',
        by rw [hC', hC]; exact hcen.1, by rw [hC', hC]; exact hcen.2⟩, hstage, hcanl,
        fun h0 => hout' hP hP' h0⟩, hrd'⟩
    exact resultS_prepend_restart (stepsAll_trans hst1 hst)
      ⟨1, c2, s2, es, c', t, hst1, hseg, hne2, hidle'⟩ (by omega) (by omega) (by rw [hC', hC])
      (by rw [hrem', hrem]) hres

/-- The countdown of an idle replay stretch, carrying the stage. -/
theorem idle_countdown3SR (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    {Rd : Control → GalilVM → Prop} (hcl : ReadyClosure raw P q first delay Rd)
    (hshape : StartShape' raw P q first delay) (hbudget : ReplayBudgetRD raw P q first delay)
    (hrs : RestartShapeL P) (hBl : B < (encoded raw).length) {r : ℕ}
    (ih : ∀ m' < r, ∀ (c : Control) (s : GalilVM) (k : ℕ), EntryR raw P q first delay B Rd c s k m' →
      Result3S raw P q first delay B c s k m') :
    ∀ (j : ℕ) (c : Control) (s : GalilVM) (k m : ℕ), m + 1 ≤ r → c.mode = .scan →
      c.replaying = true →
      c.clock = j + 1 → s.chain = ChainVM.idle → SearchReady (searchLens.get s) →
      ...
      RadiusRep s.radius k → Bnd B s →
      (GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none) →
      ReplayStageD raw P q first delay c s → Canonical s.length → Rd c s →
      (∃ (c1 : Control) (t : GalilVM),
        WatchSegE P q first delay (List.replicate j false) c s c1 t ∧
        StepsAll (galilFrameS P q first) delay (SoundScanNR raw) j ⟨c, s⟩ ⟨c1, t⟩ ∧
        c1.mode = c.mode ∧ c1.replaying = c.replaying ∧ c1.clock = 1 ∧
        ...
        t.left = s.left ∧ t.right = s.right ∧ t.center = s.center ∧
        t.replay = s.replay ∧ t.remaining = s.remaining ∧ t.radius = s.radius ∧
        ReplayStageD raw P q first delay c1 t ∧ t.length = s.length ∧ Rd c1 t) ∨
      BusyS raw P q first delay B c s k (m+1) := by
  intro j
  induction j with
  | zero =>
    intro c s k m _ hm hr hc hidle hsr _ _ _ _ _ _ _ hst _ hrd
    exact Or.inl ⟨c, s, .stop _ _, .zero _ (GalilReplaySegment.soundScanNR_replaying raw hr),
      rfl, rfl, hc, hidle, hsr, rfl, rfl, rfl, rfl, rfl, rfl, hst, rfl, hrd⟩
  | succ j ihj =>
    intro c s k m hmr hm hr hc hidle hsr hrep hM hi hfr hrad hbd hcen hst hlen hrd
    obtain ⟨v, hv⟩ := hsearch s hsr false
    have hclt : 1 < c.clock := by omega
    by_cases hf : v.search.mode = .found
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
      ...
      have hpart0 : ChainW raw (position s'.center) E (position s'.right)
        ...
        have h1 := (hbrk hl).2.2
        unfold budOf; rw [hr']; simp only; omega
      have hlive := chain_runWR raw P q first delay B E lim (P.centre s) b xs hex hd hrs hcl hBl (by omega)
        (fun h => by rw [← hBeq]; exact hlimF h) (fun h => by rw [← hBeq]; exact hlimT h)
        (m+1) {c with clock := c.clock - 1} s' k hm (by simp; omega) (by simpa using hr)
        (fun h => absurd h (Nat.succ_ne_zero m)) (by rw [hrep', hrep]) (bnd_congr hr' hrep' hbd) hpart0
        (by rw [hr']; omega) (by rw [hC']; exact fun h => ⟨(hbrk h).1, (hbrk h).2.1⟩)
      ...
      have hQ : SoundScanNR raw ⟨{c with clock := c.clock - 1}, s'⟩ :=
        GalilReplaySegment.soundScanNR_replaying raw hr
      exact Or.inr <| found_resultR hP hP' ih hidle hr
        (.bg c s s' hm hr hclt hidle hb (by rw [hget']; exact hf))
        (.succ (GalilReplaySegment.soundScanNR_replaying raw hr)
          (.scan_count c s s' hm (Or.inl hr) hclt hb) (.zero _ hQ))
        hQ (by rw [hch']; exact chainStart_ne_idle _ _ _ _ _) (by rw [hl', hr', hC']; exact hi) hlive
    ...
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
        idle_background_exists P q first s hidle hv hf
      have hrd' : Rd {c with clock := c.clock - 1} s' :=
        hcl.seg _ _ _ _ _ (.countR c s s' hm hr hclt hidle hb (.stop _ _)) hch' hrd
      have hsr' : SearchReady (searchLens.get s') := hcl.ready _ _ hrd'
      have hfld := backgroundS_fields P q first hb
      have hrem' : s'.remaining = s.remaining := hfld.2.2.2.2.2.2.2.2.1
      have hrad' : s'.radius = s.radius := hfld.2.2.2.2.2.1
      have hlen' : s'.length = s.length := hfld.2.2.2.2.2.2.1
      ...
      rcases ihj {c with clock := c.clock - 1} s' k m hmr hm hr (by simp; omega) hch' hsr'
          (by rw [hrep', hrep]) (minv_same rfl hr' hC' hrep' hM) (by rw [hl', hr', hC']; exact hi)
          (frontier_congr hr' hrep' hfr) (by rw [hrad']; exact hrad) (bnd_congr hr' hrep' hbd)
          (by rw [hC']; exact hcen) hst' (by rw [hlen']; exact hlen) hrd' with hA | hB'
      · obtain ⟨c1, t, hseg, hstp, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
          hrd1, hst1, hln1, hrdA⟩ := hA
        exact Or.inl ⟨c1, t, .countR c s s' hm hr hclt hidle hb hseg,
          .succ (GalilReplaySegment.soundScanNR_replaying raw hr)
            (.scan_count c s s' hm (Or.inl hr) hclt hb) hstp,
          hm1, hr1, hc1, hidle1, hsr1, by rw [hl1, hl'], by rw [hrr1, hr'], by rw [hC1, hC'],
          by rw [hrp1, hrep'], by rw [hrem1, hrem'], by rw [hrd1, hrad'], hst1, by rw [hln1, hlen'], hrdA⟩
      · exact Or.inr (busyS_prepend hstp1 rfl (by rw [hr']) hC' hrem' hB')

/-- The comparison of an idle replay stretch, carrying the stage; the quiet
branch exports the output soundness of the refreshed state. -/
theorem idle_compare3SR (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    {Rd : Control → GalilVM → Prop} (hcl : ReadyClosure raw P q first delay Rd)
    ...
    (hbd : Bnd B s)
    (hcen : GalilScaffoldInputTrace.Represents s.center.head raw ∧ s.center.head.focus ≠ none)
    (hst : ReplayStageD raw P q first delay c s) (hlen : Canonical s.length) (hrd : Rd c s) :
    (∃ (c1 : Control) (u : GalilVM),
      WatchSegE P q first delay [true] c s c1 u ∧
      StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩ ⟨c1, u⟩ ∧
      c1.mode = .scan ∧ c1.clock = delay ∧ c1.replaying = decide (0 < m) ∧
      ...
      u.center = s.center ∧ Frontier u ∧ u.remaining = s.remaining ∧
      RadiusRep u.radius (k+1) ∧ Bnd B u ∧
      ReplayStageD raw P q first delay c1 u ∧ Canonical u.length ∧ OutputRel raw c1 u ∧ Rd c1 u) ∨
    BusyS raw P q first delay B c s k (m+1) := by
  classical
  have hbound : position s.right + (m+1) ≤ 2 * arrived s.right := hfr (m+1) hrp
  have hav : canRight s.right := GalilReplaySegment.canRight_of_frontier (Nat.succ_pos m) hbound
  ...
  · -- a quiet comparison
    ...
    have hout : OutputRel raw {c with clock := delay, output := o, replaying := !P.replayExhausted u} u :=
      outputRel_of_refresh' raw P hP hP' q first u c.output o hiu ho _ rfl
    have hrdu : Rd {c with clock := delay, output := o, replaying := !P.replayExhausted u} u :=
      hcl.seg _ _ _ _ _
        (.matchIdleR c s vs vq o hm hr hc hav hidle rfl rfl rfl hmt hq hf ho (.stop _ _))
        (by rw [hudef, replayDec_chain, afterCompare_chain]) hrd
    refine Or.inl ⟨{c with clock := delay, output := o, replaying := !P.replayExhausted u}, u,
      .matchIdleR c s vs vq o hm hr hc hav hidle rfl rfl rfl hmt hq hf ho (.stop _ _), ?_,
      hm, rfl, hflag, ?_, ?_, ?_, hiu, hurep, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, hout, hrdu⟩
    · have ht := scan_match_idle_S' P q first delay c s vs vq o hm (Or.inl hr) hc hidle rfl rfl rfl
        hmt hq hf (by rw [hr]; exact ho)
      rw [hr] at ht
      exact .succ (GalilReplaySegment.soundScanNR_replaying raw hr) (by simpa using ht)
...
/-- **The idle replay with chain starts, breaks and restarts, carrying the
stage**, by strong induction on the comparisons left. -/
theorem replay_construct3SR (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    {Rd : Control → GalilVM → Prop} (hcl : ReadyClosure raw P q first delay Rd)
    (hshape : StartShape' raw P q first delay) (hbudget : ReplayBudgetRD raw P q first delay)
    (hrs : RestartShapeL P) (hBl : B < (encoded raw).length) :
    ∀ (r : ℕ) (c : Control) (s : GalilVM) (k : ℕ), EntryR raw P q first delay B Rd c s k r →
      Result3S raw P q first delay B c s k r := by
  intro r
  induction r using Nat.strong_induction_on with
  | _ r ih =>
    intro c s k hE
    obtain ⟨⟨⟨hm, hc, hrp, hrep, hidle, hsr, hM, hi, hfr, hrad, hbd, hcen1, hcen2⟩, hst, hlen, hout0⟩, hrd⟩ := hE
    cases r with
    | zero =>
      have hrf : c.replaying = false := by rw [hrp]; simp
      exact Or.inl ⟨[], c, s, .stop _ _, fun hlast => .zero _ hlast, by simp, by simp,
    ...
    | succ n =>
      have hrt : c.replaying = true := by rw [hrp]; simp
      rcases idle_countdown3SR (q := q) (first := first) hP hP' hex hd hsearch hcl hshape hbudget hrs
          hBl ih (delay - 1) c s k n le_rfl hm hrt (by omega) hidle hsr hrep hM hi hfr hrad hbd
          ⟨hcen1, hcen2⟩ hst hlen hrd with hA | hB1
      · obtain ⟨c1, t1, hseg1, hstp1, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
          hrd1, hst1, hln1, hrdA⟩ := hA
        have hm1' : c1.mode = Mode.scan := by rw [hm1, hm]
        have hr1' : c1.replaying = true := by rw [hr1, hrt]
        ...
        have hi1 : ScanInvariant raw (position t1.center) k t1.left t1.right := by
          rw [hl1, hrr1, hC1]; exact hi
        have hfr1 : Frontier t1 := frontier_congr hrr1 hrp1 hfr
        rcases idle_compare3SR (q := q) (first := first) hP hP' hex hd hsearch hcl hshape hbudget hrs
            hBl ih c1 t1 k n le_rfl hm1' hr1' hc1 hidle1 hsr1 hM1 hi1 (by rw [hrp1, hrep]) hfr1
            (by rw [hrd1]; exact hrad) (bnd_congr hrr1 hrp1 hbd)
            (by rw [hC1]; exact ⟨hcen1, hcen2⟩) hst1 (by rw [hln1]; exact hlen) hrdA with hA2 | hB2
        · obtain ⟨c2, u, hseg2, hstp2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2,
            hrem2, hrad2, hbd2, hst2, hlen2, hout2, hrd2⟩ := hA2
          have hres := ih n (by omega) c2 u (k+1)
            ⟨⟨⟨hm2, hc2, hr2, hrep2, hidle2, hsr2, hM2, hi2, hfr2, hrad2, hbd2,
              by rw [hC2, hC1]; exact hcen1, by rw [hC2, hC1]; exact hcen2⟩, hst2, hlen2,
              fun _ => hout2⟩, hrd2⟩
...
`RestartShapeL` (true of `restartVM`, `restartShapeL_sharedC`).  Branch (iii)
additionally reports the output soundness at the idle landing. -/
theorem replay_after_fallback_general''_R (raw : List (Fin 2)) (P : Shared)
    (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM) (q : ℕ) (first : Fin 9)
    (delay : ℕ) (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
  ...
  have hst0 : ReplayStageD raw P q first delay c t :=
    ⟨t, 0, reset, [], c, hR, stageEntry_zero reset, hc, .stop _ _⟩
  rcases replay_construct3SR (q := q) (first := first) hP hP' hex hd hsearch hcl hshape hbudget hrs hB
      r c t 0
      ⟨⟨⟨hm, hc, by rw [hrpl]; simp [hr0], hrep, hR.1, searchReady_of_restarted hR, hM,
        hR.2.2.2.1, hfr, hR.2.2.2.2.1, hbd, hR.2.1, hR.2.2.1⟩, hst0, hR.2.2.2.2.2.1,
        fun h0 => absurd h0 (by omega)⟩, hcl.restart c t 0 reset hm hc hR (stageEntry_zero reset)⟩
      with hQ | hBusy
  · obtain ⟨es, c', t', hseg, hst, hlen, hcnt, ⟨hm', hc', hr', hrep', hidle', hsr', hM', hi', hpos', hC',
      hfr', hrr', hrem'⟩, -⟩ := hQ
    exact Or.inl ⟨es, c', t', hseg, hst, hlen, hcnt, hpos', hC',
...
/-- `replay_after_fallback_general''_fuel` at `delay = 2048` for a decoding
machine: no chain-start hypothesis at all. -/
theorem replay_after_fallback_general''_R_of_decodes (raw : List (Fin 2)) (P : Shared)
    (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM) (q : ℕ) (first : Fin 9)
    (hex : ∀ s, P.replayExhausted s = zero s.replay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    ...
    (BrokeAndRestarted raw P q first 2048 c t ∧
      ∃ (n : ℕ) (c' : Control) (t' : GalilVM),
        ...
        position t'.right = position t.right + r ∧ t'.center = t.center ∧
        GalilReplaySegment.InvScan 2048 raw c' t' r ∧ OutputRel raw c' t') :=
  replay_after_fallback_general''_R raw P hP hP' q first 2048 hex (by omega) hsearch hcl
    (startShape'_of_decodes hdec) hbudget hrs r hr0 c t hm hc hrpl hR hrep hM hfr hsi
...
#print axioms chain_breakWR
#print axioms replay_construct3SR
#print axioms replay_after_fallback_general''_R
#print axioms replay_after_fallback_general''_R_of_decodes
...
/-! ## 24. The replay construction over the reachability-restricted readiness leaf

`hpres` (every search effect of every state preserves `SearchReady`) is refuted
by `GalilLeafPres.hpres_false_at`.  What the readiness layer can actually
deliver (`CloseoutPreload41.HpresRepAt`) is the same law only at the states of
an actual `SoundScanNR` run out of the cycle entry, and only under the clock
guard the comparison branch really has.  `HpresRun` below is exactly that,
stated here so the construction can be restated over it; it is closed under
taking `StepsAll`-successors (`hpresRun_mono`), which is all the threading the
three inductions need.  The `R` variants are otherwise verbatim copies. -/
...
/-- **The readiness leaf, restricted to the states of a run.**  `CloseoutPreload41.HpresRepAt`
spelled out without the downstream definitions (which import this file). -/
def HpresRun (raw : List (Fin 2)) (P : Shared) (q : ℕ) (first : Fin 9) (delay : ℕ)
    (x : State GalilVM) : Prop :=
  ∀ (k : ℕ) (y : State GalilVM),
    StepsAll (galilFrameS P q first) delay (SoundScanNR raw) k x y →
    y.ctl.mode = Mode.scan →
    ∀ (a : Bool) (v : SearchVM), y.vm.chain = ChainVM.idle → (a = true → y.ctl.clock ≤ 1) →
      searchEffect P a y.vm v → SearchReady v

/-- `HpresRun` travels along the run. -/
theorem hpresRun_mono {x y : State GalilVM} {k : ℕ}
    (h : HpresRun raw P q first delay x)
    (hst : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) k x y) :
    HpresRun raw P q first delay y :=
  fun k' z hst' => h (k + k') z (stepsAll_trans hst hst')

theorem found_resultRP (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    {c c2 : Control} {s s2 : GalilVM} {k r r0 k2 m2 : ℕ} {cc b : Fin 3} {xs : List (Fin 3)}
    (ih : ∀ m' < r0, ∀ (c : Control) (s : GalilVM) (k : ℕ),
      HpresRun raw P q first delay ⟨c, s⟩ → Entry raw delay B c s k m' →
      Result3 raw P q first delay B c s k m')
    (hpresR : HpresRun raw P q first delay ⟨c, s⟩)
    (hidle : s.chain = .idle) (hrt : c.replaying = true)
    (hft : FoundTick P q first delay c s c2 s2)
    (hst1 : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩ ⟨c2, s2⟩)
    (hQ2 : SoundScanNR raw ⟨c2, s2⟩) (hne2 : s2.chain ≠ .idle)
  ...
  · obtain ⟨es, c', t, m', hseg, hlt, hm', hc', hr', hrep', hidle', hsr', hM', hi', hrad', hpos', hC',
    ...
    obtain ⟨n, hst⟩ := replayChainSeg2_stepsAll raw hP hP' hseg (position s2.center) k2 hi2 hQ2
    have hres := ih m' (by omega) c' t (k2 + (m2 - m'))
      (hpresRun_mono hpresR (stepsAll_trans hst1 hst))
      ⟨hm', hc', hr', hrep', hidle', hsr', hM', hi', hfr', hrad', hbd',
        by rw [hC', hC]; exact hcen.1, by rw [hC', hC]; exact hcen.2⟩
    exact result_prepend_restart (stepsAll_trans hst1 hst)
      ⟨1, c2, s2, es, c', t, hst1, hseg, hne2, hidle'⟩ (by omega) (by omega) (by rw [hC', hC])
...
theorem idle_countdown3RP (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hshape : GalilWatchOkInst.StartShape P) (hbudget : ReplayBudget raw P delay) (hrs : RestartShape P)
    (hBl : B < (encoded raw).length) {r : ℕ}
    (ih : ∀ m' < r, ∀ (c : Control) (s : GalilVM) (k : ℕ),
      HpresRun raw P q first delay ⟨c, s⟩ → Entry raw delay B c s k m' →
      Result3 raw P q first delay B c s k m') :
    ∀ (j : ℕ) (c : Control) (s : GalilVM) (k m : ℕ), m + 1 ≤ r →
      HpresRun raw P q first delay ⟨c, s⟩ → c.mode = .scan →
      c.replaying = true →
      c.clock = j + 1 → s.chain = ChainVM.idle → SearchReady (searchLens.get s) →
      s.replay = ofNat (m+1) → MInv raw c s →
      ScanInvariant raw (position s.center) k s.left s.right → Frontier s →
  ...
  induction j with
  | zero =>
    intro c s k m _ _ hm hr hc hidle hsr _ _ _ _ _ _ _
    exact Or.inl ⟨c, s, .stop _ _, .zero _ (GalilReplaySegment.soundScanNR_replaying raw hr),
      rfl, rfl, hc, hidle, hsr, rfl, rfl, rfl, rfl, rfl, rfl⟩
  | succ j ihj =>
    intro c s k m hmr hpresR hm hr hc hidle hsr hrep hM hi hfr hrad hbd hcen
    obtain ⟨v, hv⟩ := hsearch s hsr false
    have hclt : 1 < c.clock := by omega
    by_cases hf : v.search.mode = .found
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
      ...
      have hQ : SoundScanNR raw ⟨{c with clock := c.clock - 1}, s'⟩ :=
        GalilReplaySegment.soundScanNR_replaying raw hr
      exact Or.inr <| found_resultRP hP hP' ih hpresR hidle hr
        (.bg c s s' hm hr hclt hidle hb (by rw [hget']; exact hf))
        (.succ (GalilReplaySegment.soundScanNR_replaying raw hr)
          (.scan_count c s s' hm (Or.inl hr) hclt hb) (.zero _ hQ))
        hQ (by rw [hch']; exact chainStart_ne_idle _ _ _ _ _) (by rw [hl', hr', hC']; exact hi) hlive
    ...
    · obtain ⟨s', hb, hch', hl', hr', hC', hrep', hget'⟩ :=
      ...
      have hsr' : SearchReady (searchLens.get s') := by
        rw [hget']
        exact hpresR 0 ⟨c, s⟩ (.zero _ (GalilReplaySegment.soundScanNR_replaying raw hr)) hm false v
          hidle
          (by simp) hv
      have hrem' : s'.remaining = s.remaining :=
        (backgroundS_fields P q first hb).2.2.2.2.2.2.2.2.1
      ...
      have hst1 : StepsAll (galilFrameS P q first) delay (SoundScanNR raw) 1 ⟨c, s⟩
        ...
        .succ (GalilReplaySegment.soundScanNR_replaying raw hr)
          (.scan_count c s s' hm (Or.inl hr) hclt hb) (.zero _ hQ')
      rcases ihj {c with clock := c.clock - 1} s' k m hmr (hpresRun_mono hpresR hst1) hm hr
          (by simp; omega) hch' hsr'
          (by rw [hrep', hrep]) (minv_same rfl hr' hC' hrep' hM) (by rw [hl', hr', hC']; exact hi)
          (frontier_congr hr' hrep' hfr) (by rw [hrad']; exact hrad) (bnd_congr hr' hrep' hbd)
          (by rw [hC']; exact hcen) with hA | hB'
      · obtain ⟨c1, t, hseg, hst, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
...
theorem idle_compare3RP (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hshape : GalilWatchOkInst.StartShape P) (hbudget : ReplayBudget raw P delay) (hrs : RestartShape P)
  ...
  · -- a quiet comparison
    ...
    · have hgetU : searchLens.get u = vq := by rw [hudef, replayDec_search]; rfl
      rw [hgetU]
      exact hpresR 0 ⟨c, s⟩ (.zero _ (GalilReplaySegment.soundScanNR_replaying raw hr)) hm true
        vq hidle (fun _ => show c.clock ≤ 1 from hc.le) hq
    · exact minv_matchR P hex o delay hr rfl hav hi hM
    · rw [hur]; exact hpos
    · rw [hudef, replayDec_center, afterCompare_center]
    · intro m' hm'
...
theorem replay_construct3RP (hP : P.onLetter = onLetterVM raw) (hP' : P.leftFirst = leftFirstVM)
    (hex : ∀ s, P.replayExhausted s = zero s.replay) (hd : 1 ≤ delay)
    (hsearch : ∀ s : GalilVM, SearchReady (searchLens.get s) → ∀ a : Bool,
      ∃ v, searchEffect P a s v)
    (hshape : GalilWatchOkInst.StartShape P) (hbudget : ReplayBudget raw P delay)
    ...
    ∀ (r : ℕ) (c : Control) (s : GalilVM) (k : ℕ),
      HpresRun raw P q first delay ⟨c, s⟩ → Entry raw delay B c s k r →
      Result3 raw P q first delay B c s k r := by
  intro r
  induction r using Nat.strong_induction_on with
  | _ r ih =>
    intro c s k hpresR hE
    obtain ⟨hm, hc, hrp, hrep, hidle, hsr, hM, hi, hfr, hrad, hbd, hcen1, hcen2⟩ := hE
    cases r with
    | zero =>
      have hrf : c.replaying = false := by rw [hrp]; simp
    ...
    | succ n =>
      have hrt : c.replaying = true := by rw [hrp]; simp
      rcases idle_countdown3RP (q := q) (first := first) hP hP' hex hd hsearch hshape hbudget hrs
          hBl ih (delay - 1) c s k n le_rfl hpresR hm hrt (by omega) hidle hsr hrep hM hi hfr hrad hbd
          ⟨hcen1, hcen2⟩ with hA | hB1
      · obtain ⟨c1, t1, hseg1, hst1, hm1, hr1, hc1, hidle1, hsr1, hl1, hrr1, hC1, hrp1, hrem1,
          hrd1⟩ := hA
        have hm1' : c1.mode = Mode.scan := by rw [hm1, hm]
        ...
        have hi1 : ScanInvariant raw (position t1.center) k t1.left t1.right := by
          rw [hl1, hrr1, hC1]; exact hi
        have hfr1 : Frontier t1 := frontier_congr hrr1 hrp1 hfr
        rcases idle_compare3RP (q := q) (first := first) hP hP' hex hd hsearch hshape hbudget hrs
            hBl ih c1 t1 k n le_rfl (hpresRun_mono hpresR hst1) hm1' hr1' hc1 hidle1 hsr1 hM1 hi1 (by rw [hrp1, hrep]) hfr1
            (by rw [hrd1]; exact hrad) (bnd_congr hrr1 hrp1 hbd)
            (by rw [hC1]; exact ⟨hcen1, hcen2⟩) with hA2 | hB2
        · obtain ⟨c2, u, hseg2, hst2, hm2, hc2, hr2, hidle2, hsr2, hM2, hi2, hrep2, hpos2, hC2, hfr2,
            hrem2, hrad2, hbd2⟩ := hA2
          have hres := ih n (by omega) c2 u (k+1)
            (hpresRun_mono hpresR (stepsAll_trans hst1 hst2))
            ⟨hm2, hc2, hr2, hrep2, hidle2, hsr2, hM2, hi2, hfr2, hrad2, hbd2,
              by rw [hC2, hC1]; exact hcen1, by rw [hC2, hC1]; exact hcen2⟩
          have hlen : (List.replicate (delay - 1) false ++ [true]).length = delay := by
            simp; omega
...
theorem replay_after_fallback_general''RP (raw : List (Fin 2)) (P : Shared)
    ...
    (hR : Restarted raw t 0 reset) (hrep : t.replay = ofNat r)
    (hM : MInv raw c t) (hfr : Frontier t) (hsi : ShiftIdle t)
    (hpresR : HpresRun raw P q first delay ⟨c, t⟩) :
    (∃ (es : List Bool) (c' : Control) (t' : GalilVM),
      WatchSegE P q first delay es c t c' t' ∧
      (SoundScanNR raw ⟨c', t'⟩ →
        StepsAll (galilFrameS P q first) delay (SoundScanNR raw) es.length ⟨c, t⟩ ⟨c', t'⟩) ∧
  ...
  have hbd : Bnd (position t.right + r) t := by
    intro m hm'
    rw [ofNat_inj (hrep.symm.trans hm')]
  rcases replay_construct3RP (q := q) (first := first) hP hP' hex hd hsearch hshape hbudget hrs hB
      r c t 0 hpresR
      ⟨hm, hc, by rw [hrpl]; simp [hr0], hrep, hR.1, searchReady_of_restarted hR, hM,
        hR.2.2.2.1, hfr, hR.2.2.2.2.1, hbd, hR.2.1, hR.2.2.1⟩ with hQ | hBusy
  · obtain ⟨es, c', t', hseg, hst, hlen, hcnt, hm', hc', hr', hrep', hidle', hsr', hM', hi', hpos', hC',
...
#print axioms HpresRun
#print axioms hpresRun_mono
#print axioms found_resultRP
#print axioms idle_countdown3RP
#print axioms idle_compare3RP
#print axioms replay_construct3RP
