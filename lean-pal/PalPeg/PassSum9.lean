...
**巡回配置の構成的探索**（`u = W^m · v · W^n`、`W` 原始的、`m ∈ {8,9,10}`、`n ∈ {1,2}`、
`v` は構造化＋乱択、回転 `ρ` を全探索）：有効な配置 3,722 個すべてで
**領域内のパスは節点 1 個だけ**（`t_j` 到達 0 件、深い跨ぎ 0 件、最大 `C/p_c = 0.217`）。
理由：run 1 `= W^m` の直後の着地点 `t₂ = m*P - k*P + 1` からは `W` の周期性が
ちょうど `k*P - 1` セルしか残らない（`k`-繰り返しに 1 セル足りない）ため、
`t₂` では周期 `P` を取れず、`W` は原始的なので小さい周期も無い。
したがって `t₂` の節点は `v` を跨ぐ大きい周期を取り、その run は領域を出てしまう。
