...
namespace PalPeg.SlotClockEvents
...
def level (q : SlotClockStartup.Control) : Flags := fun i =>
  decide (q.1 = 32 ∧ (q.2 i).2 = 14)

/-- Sample phase 14 at the beginning of each actual arrival. Its rising edge
...
theorem entry_step (s : SlotSchedule.SlotSt) :
    ((SlotSchedule.slotStep s).p = 14 ∧ s.p ≠ 14) ↔
      ((SlotSchedule.slotStep s).p = 14 ∧ (SlotSchedule.slotStep s).r = 0) := by
  ...
  · by_cases hp : s.p + 1 < 30
    · simp only [SlotSchedule.slotStep, hr, hp, ↓reduceIte]
      ...
      · rintro ⟨h, _⟩; exact ⟨h, by omega⟩
    · simp [SlotSchedule.slotStep, hr, hp]

theorem restart_step (s : SlotSchedule.SlotSt) (hq : 2 ≤ s.q) :
    ((SlotSchedule.slotStep s).p = 14 ∧ (SlotSchedule.slotStep s).r = 1) ↔
      (s.p = 14 ∧ s.r = 0) := by
  by_cases hr : s.r + 1 < s.q
  · simp only [SlotSchedule.slotStep, hr, ↓reduceIte]
    omega
...
theorem canonical_edge (n : ℕ) (hn : 32 ≤ n) (i : Fin 4) :
    (SlotSchedule.phaseAt (n + 1) i = 14 ∧ SlotSchedule.phaseAt n i ≠ 14) ↔
      (SlotSchedule.phaseAt (n + 2) i = 14 ∧ SlotSchedule.resAt (n + 2) i = 1) := by
...
theorem level_schedule (t : ℕ) (i : Fin 4) :
    level (atTime (32 + t)).state.1.1 i = decide (SlotSchedule.phaseAt (32 + t) i = 14) := by
...
theorem pulse_later (t : ℕ) (i : Fin 4) :
    (atTime (34 + t)).state.2.2 i = SlotSchedule.rsCanon (34 + t) i := by
  rw [show 34 + t = (32 + t) + 2 by omega, delayed_edge]
  have h1 := level_schedule (t + 1) i
  rw [show 32 + (t + 1) = (32 + t) + 1 by omega] at h1
  rw [h1, level_schedule, SlotSchedule.rsCanon_iff_phase (by omega)]
...
theorem pulse_initial (i : Fin 4) :
    (atTime 32).state.2.2 i = SlotSchedule.rsCanon 32 i ∧
    (atTime 33).state.2.2 i = SlotSchedule.rsCanon 33 i := by
  fin_cases i <;> decide

theorem pulse_matches (n : ℕ) (hn : 32 ≤ n) (i : Fin 4) :
    (atTime n).state.2.2 i = SlotSchedule.rsCanon n i := by
  by_cases h32 : n = 32
...
/-- info: 'PalPeg.SlotClockEvents.pulse_matches' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms pulse_matches

/-- info: 'PalPeg.SlotClockEvents.run_project' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms run_project
/-- info: 'PalPeg.SlotClockEvents.delayed_edge' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms delayed_edge

end PalPeg.SlotClockEvents
