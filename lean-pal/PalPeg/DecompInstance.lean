...
/-- 主テープのうち `X`/`X2`/`F` の 3 本は埋め込んだ動作列で一切動かない。
`Cnt`/`U`/`P` は第 2 相の比較カウンタ `Ca`/`Cb`/`Cc` の置き場所に使うので除く
