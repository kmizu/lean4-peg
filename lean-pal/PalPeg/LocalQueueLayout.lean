...
/-!
# The queue of the local machine: finite observation and sealed layout

The finite observation `QueueView` that `CloseoutCoreEnc25.deltaOf` / `tagStep` actually read, the
sealed layout `LaysSealed` (junk starts with the unused cell `none`) from which the observation
is read off the two top cells, and the sub-step on cells that keeps the layout.
-/
