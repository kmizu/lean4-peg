...
`PAL` **is** in PEG, and this is now proved in Lean 4 with no hypothesis:
`PalPeg.PalInPeg.unconditional : RecognizedByTotalPEG PAL` in
[`proof/`](https://github.com/kmizu/pal-peg/tree/main/proof) of [kmizu/pal-peg](https://github.com/kmizu/pal-peg) uses only the standard axioms.
The proof builds a *scaffolding automaton* (SCA) for `PAL` and turns it into a total PEG with the
SCA → PEG theorem of Kim & Park (2026, arXiv:2608.29592; Lean artifact, Zenodo 22099762), which
formalizes the characterization of Loff–Moreira–Reis. Since `reverse(PAL) = PAL`, `PAL` is in PEG.
As a corollary, LMR's Conjecture 7 ("even-length palindromes are not in PEG") is false.

The automaton is the *window-pal* construction: two dyadic stages, each with a Galil–Seiferas
real-time string matcher and a batch of palindrome flags. The Scala generator in
[`generator/`](https://github.com/kmizu/pal-peg/tree/main/generator) prints an explicit ordinary PEG for the
same automaton through its own SCA → PEG translator. That printed grammar is tested on finite inputs
but not verified: the Lean theorem gets its PEG from Kim & Park's construction instead.
...
mismatch — so PAL has an explicit, executable, exhaustively verified grammar in Macro PEG. For
plain PEG, see the Status section above.
...
* [kmizu/pal-peg](https://github.com/kmizu/pal-peg): [`proof/`](https://github.com/kmizu/pal-peg/tree/main/proof) (the Lean proof) and
  [`generator/`](https://github.com/kmizu/pal-peg/tree/main/generator) (the Scala generator).
