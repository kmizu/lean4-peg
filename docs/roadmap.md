...
- **T7 追撃で反例候補にしていた偶数長回文は PEL の側にある**: Galil 1978（real-time 多テープ TM による
  ...
  合成すると PAL ∈ PEG。LMR の Conjecture 7 はこの合成で偽になる。その後、仮定なしの Lean 証明
  （Kim–Park のアーティファクト上、標準公理のみ）を [kmizu/pal-peg](https://github.com/kmizu/pal-peg) に置いた。
