...
| 定理 | ファイル | 仮定 |
|---|---|---|
| `pal_in_peg_final12` | `CloseoutPackRun9` | `BigResid5O`（6 契約）, `H_extraEntry/Tick`, `H_shiftLocalC`, `H_stageScan`, `CycleOracleMC3`, `H_bootShift`, `H_landShift`, `H_realizeLIO'`（最上位。found 葉は `foundExit_compare_final9`、readiness は `PostRunP`、核は `chooseVm_tapeActK`；詳細 `CLAUDE_RESUME.md` n35） |
