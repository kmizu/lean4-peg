...
| 定理 | ファイル | 仮定 |
|---|---|---|
| `pal_in_peg_final15` | `CloseoutPackRun18` | `BigResid6`（6 契約）, `H_extraEntry3/Tick3`（rewind 角なし）, `H_marksEntry'`, `H_shiftLocalC`, `H_stageScan`, `CycleOracleMC3`, `H_bootShift`, `H_landShift`, `H_realizeLIM'`（最上位。found 葉は `foundExit_compare_final9`、readiness は `PostRunC`、核は `chooseVm_tapeActK`；詳細 `CLAUDE_RESUME.md` n48；core `shiftVm_tapeActKQ` K=28；`ShiftLocal` は scan ガード要（PackRun24 警告）；`BigResid6` は `LPackM2` 4 場 + `rShiftNext`；readiness は `StageInvD` で `.double` 決着；rewind 角は `MarksEntry` 1 点；found 葉は `foundExit_compare_final11`（`WatchPrefixC` は定理）；`rewindMargin` は `CentreMargin` 1 葉に集約、`.double` 出口義務は Scala に合わせ再定式化要） |
