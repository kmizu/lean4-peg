...
| 定理 | ファイル | 仮定 |
|---|---|---|
| `pal_in_peg_final14` | `CloseoutPackRun14` | `BigResid6`（6 契約）, `H_extraEntry''/Tick''`（`CentreMargin`）, `H_shiftLocalC`, `H_stageScan`, `CycleOracleMC3`, `H_bootShift`, `H_landShift`, `H_realizeLIM'`（最上位。found 葉は `foundExit_compare_final9`、readiness は `PostRunC`、核は `chooseVm_tapeActK`；詳細 `CLAUDE_RESUME.md` n41；rewind 角は `MarksEntry` 1 点；found 葉は `foundExit_compare_final11`（`WatchPrefixC` は定理）；`rewindMargin` は `CentreMargin` 1 葉に集約、`.double` 出口義務は Scala に合わせ再定式化要） |
